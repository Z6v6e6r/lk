...
### Блок «Данные ЛК устарели» и молчаливая ошибка статуса (2026-09-12)

- Блок `#lk-idle-data-guard` создаёт общий `src/utils/lkIdleDataGuard.ts`, а его стили лежат только в `src/MyApp.css`. На Tilda-странице стили кабинета не подключены, поэтому витрина показывала голую строку внизу. В бандл добавлен `src/components/subscription-storefront/storefront-idle-guard.css` — копия кабинетного блока из `MyApp.css` (плюс `@font-face 'RF Dewi UltraBold'` из связанного шрифта витрины и `--btn-font`), импортируется в `src/subscription-storefront.tsx`. Регрессионный тест сравнивает копию с исходником построчно (нормализованные пробелы).
- Проверка отрисовки в локальном preview: панель `rgb(255,255,255)` радиус 20px, фон `rgba(18,16,24,0.58)` с блюром, кнопка `rgb(115,83,217)` радиус 16px высота 52px, заголовок в `RF Dewi UltraBold` — как на остальных страницах LK1.
- Сбой обновления каталога больше не показывается: убраны текст «Не удалось обновить подписки…» и кнопка «Повторить», `role="alert"` не рендерится. Карточки остаются с последними известными данными, повтор делает существующий 30-секундный цикл. Состояния первой загрузки и пустого каталога сохранены.
- Харнесс получил сценарий `?fail=status` (статус отвечает 500) и проверку `checkSilentStatusFailure`: за 30 секунд ожидания статус-запрос был вызван, ошибка/ретрай/alert не появились, «Загружаем подписки…» пропала. Все четыре сценария harness — PASS.
- Проверки: focused 14/14, TypeScript, ESLint, prod/dev сборки — PASS.
...
## Клубная страница «ДРУЖБА.АТЛАНТЫ» (2026-09-21)

- Новый T123-блок: `docs/tilda-atlanty-subscription.html`. Он монтирует ту же витрину
  `LKWidgetSubscriptionStorefront` с `data.variant="atlanty"` (резервный способ —
  `?variant=atlanty`), поэтому URL страницы не обязан содержать параметры. Шапка и
  навигация скрыты CSS самого блока, как в присланном встраиваемом блоке
  `блок_подписок_основной.html`; `docs/tilda-subscription-storefront.html` их не скрывает.
- Блок fail-closed по версии бандла: перед mount он проверяет
  `LKWidgetSubscriptionStorefront.storefrontVariants` на `atlanty`. Старый бандл без этого
  флага показывает сообщение об обновлении и **не** рендерит обычный каталог
  (Дружба/РА/Академия/Энергия) вместо клубной карточки.
- Вариант `atlanty` рисует одну карточку «ДРУЖБА.АТЛАНТЫ»: плоский текстовый заголовок
  вместо общего бейджа «Дружба», преимущества и цвет — как у «Дружбы». Заглушка
  «месяц 2 часа» в этом варианте отсутствует; обычная витрина `/sub_hab` не меняется.
- Два варианта покупки — прямые продукты Viva (`apiBuySubscroption`), как
  «РА»/«Академия»/«Энергия 5»:
  - «месяц»: 6 800 ₽ вместо зачёркнутых 9 800 ₽, продукт
    `3907d127-a6b0-419e-a933-4a2857f26356`;
  - «год»: 68 000 ₽ вместо зачёркнутых 98 000 ₽, id продукта ещё не выдан
    оператором — кнопка видна, но закрыта («Скоро»).
- Пустой/пробельный id любого клубного варианта закрывает покупку:
  `resolveStorefrontBillingTarget('atlanty', …)` возвращает `null`, поэтому нет
  «провала» в counter-контур `apiCreateTournamentSubscriptionPurchase`.
- Создание прямого продукта отправляется с `retries: 0`: у контракта провайдера нет
  ключа идемпотентности, повтор неоднозначного create мог бы создать вторую транзакцию.
- Цены на этой странице статические: у клубных прямых продуктов нет собственного
  счётчика статуса. Показывается операторская цена, фактическую сумму по-прежнему
  определяет провайдер/банк. При изменении цены обновить `ATLANTY_*_PRICE_MINOR` и
  `ATLANTY_*_COMPARE_MINOR` в `src/components/subscription-storefront/catalog.ts` и
  пересобрать бандл.
- Годовой вариант включается заменой `ATLANTY_ANNUAL_PRODUCT_ID` (пустая строка →
  точный id продукта) и новой сборкой.
- Авторизация и оплата штатные: гость видит общий `AuthForm`, после входа кнопка
  «Продолжить оплату» создаёт транзакцию и уводит на `paymentUrl` банка; возврат
  подтверждается через `summerPaymentRef`. Для годового варианта остаётся согласие
  с условиями (тот же гейт, что у годовой «Дружбы»).
- LOCAL: `node --experimental-strip-types --test scripts/tests/subscriptionStorefront.test.ts`
  — 22/22 PASS; `npx tsc -b` PASS; ESLint изменённых файлов PASS; prod/dev сборки
  `subscription-storefront` PASS; в собранном бандле присутствуют `ДРУЖБА.АТЛАНТЫ`,
  id продукта и флаг `storefrontVariants`.
- Headless Chrome smoke (локальная сборка, временный фикстур удалён): одна карточка
  «ДРУЖБА.АТЛАНТЫ»; «месяц» → 9 800 ₽ зачёркнуто / 6 800 ₽ / «Оформить подписку»;
  «год» → 98 000 ₽ зачёркнуто / 68 000 ₽ / «Скоро» disabled + пояснение.
- Остаточные риски (не закрыты кодом):
  - у клубных продуктов нет серверного счётчика, поэтому доступность/привязка/остаток
    не проверяются до клика: неактивный или несвязанный продукт отдаст ошибку провайдера
    только после нажатия «Оформить подписку». Перед публикацией нужно один раз проверить
    на dev, что продукт активен и привязан;
  - подтверждение после банка для прямых продуктов (как у РА/Академии/Энергии) не
    сохраняет pending-запись и отправляет `counterKey: null`; нужно проверить на dev,
    что `confirm` возвращает `PAID` и клиент уходит в кабинет;
  - статические цены не сверяются с суммой провайдера автоматически;
  - блок нельзя ставить на страницу, где уже есть обычный блок витрины: оба используют
    `id="padlhub-subscriptions"`, второй mount размонтирует первый.
- НЕ выполнено: Tilda-страница не создавалась и не публиковалась, бандл не выкладывался,
  боевая транзакция по новому продукту не создавалась, годовой id ещё не задан.
  Блок начнёт показывать клубную карточку только после публикации новой сборки
  `subscription-storefront` и её `release.json`/`release-dev.json`.

## Клубная подписка «Дружба Топократы» (2026-09-24)
