...
- Новый T123-блок: `docs/tilda-atlanty-subscription.html`. Он монтирует ту же витрину
  ...
  навигация витрины в блоке скрыты, как в основном блоке подписок.
...
- Годовой вариант включается заменой `ATLANTY_ANNUAL_PRODUCT_ID` (пустая строка →
  точный id продукта) и новой сборкой. До этого
  `resolveStorefrontBillingTarget('atlanty', 'annual')` возвращает `null`, поэтому
  оплата годового варианта невозможна даже в обход UI.
...
- LOCAL: `node --experimental-strip-types --test scripts/tests/subscriptionStorefront.test.ts`
  — 21/21 PASS; `npx tsc -b` PASS; ESLint изменённых файлов PASS; prod/dev сборки
  `subscription-storefront` PASS; в собранном бандле присутствуют `ДРУЖБА.АТЛАНТЫ` и
  id продукта.
