...
партии по 50 подписок, чекбокс согласия, штатную авторизацию и переворачиваемую
...
До заполнения product ID всех ценовых партий и публикации managed policy
продажи и использование подписки должны оставаться выключенными.
...
`kotelniki_friendship`: четыре ценовые партии по 100 подписок, суммарный
fallback-лимит 400. Цены на изображениях партий: 19 800 / 23 800 / 36 800 /
56 800 ₽.

Страница всей сети монтирует вариант и отдельный counter key
`network_friendship`: одна партия из 50 подписок, fallback-лимит 50. На
