...
DEV-проверка опубликованных файлов использует проверенный HTTPS-адрес
`https://lk-reserve.89-108-64-209.sslip.io/lk/` и открывает отдельное соединение для каждого
GET: синхронный SCP может задержать обработку закрытия старого keep-alive соединения.
Проверка после публикации и после отката сохраняет 20-секундный предел на весь ответ,
проверку TLS, всех 12 хешей и запрет кэширования манифеста. При сетевой ошибке журнал
указывает имя файла и код транспорта; проверка не пропускается и не повторяется автоматически.
