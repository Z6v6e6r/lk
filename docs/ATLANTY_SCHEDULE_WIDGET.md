...
| `typeId` | `type.id` из Viva — нужен попапу записи, чтобы он видел событие; на фильтрацию влияет только у категории без `directionId` |
...
**Как событие попадает в категорию.** Сначала по направлению: событие относится к
категории, если его `direction.id` совпадает с `directionId` категории. Только если
у категории направления нет, работает признак `typeId`. Это защищает от чужого
клуба: тип `2349` «Корпоративные клиенты» общий, например, у «Атлантов» (6152) и
у «Топократов» (6180, 6233), поэтому по типу события разных клубов не смешиваются.
...
Пул — 28 клубных фото, те же, что LK2 раскладывает по карточкам рекомендаций
...
(`skolkovo-*`, `premium-*`, `hero-*`, `card-art-*`, `promo*`).

Файлы лежат в `https://padlhub.su/lk/atlanty-cards/v2/`, все — WebP шириной до
800 px (суммарно 894 КБ). Исходники LK2 были 1200×480 по 82–101 КБ и два PNG
(716 КБ и 165 КБ): карточка показывает фото шириной ~320–400 px, поэтому
«как есть» четыре видимые карточки стоили ~370 КБ, а с PNG в перемешанном пуле
могли стоить больше мегабайта. После пересжатия четыре карточки — ~150 КБ.
Два превью `nagatinskaya-*` (171×100) в пул не входят: на шапке они мыльные.
