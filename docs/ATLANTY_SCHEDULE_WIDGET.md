...
1. Добавить блок **T123 («HTML-код»)**.
2. Вставить содержимое `docs/tilda-atlanty-schedule.html` целиком.
3. Опубликовать страницу.

Вставка состоит из трёх частей: конфиг категорий, загрузчик бандла (с версией из
`/lk/release.json` для cache-bust) и инициализация попапа записи Viva отдельным
инстансом `atlanty`. Если на странице уже есть виджет Viva с другим инстансом —
конфликта нет.
...
- Клик по карточке открывает модалку; попап записи Viva вызывается кнопкой
  «Записаться». Предзаполнение события в попапе опирается на контракт виджета Viva
  (`#atlanty&exerciseId=…`); при смене их бандла возможен откат к открытию
  расписания без фокуса на событии.
