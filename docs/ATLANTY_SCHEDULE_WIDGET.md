...
открывает карточку события с кнопкой «Записаться»: по умолчанию — попап записи
Viva, в режиме `booking: "lk"` — окно записи ЛК1 с контуром ограничений.
...
window.LK_ATLANTY_SCHEDULE_CONFIG = {
  ...
  booking: "viva"       // "viva" — попап VivaCRM, "lk" — окно записи ЛК1
};
```
...
| Опция | Значения |
| --- | --- |
| `booking` | `"viva"` (по умолчанию) — карточка открывает официальный попап VivaCRM; `"lk"` — окно записи ЛК1 с входом, вариантами оплаты и серверным контуром ограничений |
| `bookingAllowedTypeIds` | типы занятий (Viva `type.id`), которые окно ЛК1 должно видеть; по умолчанию — `typeId` категорий. Нужно для витрин вне клубного расписания `/group` |
| `bookingDirectionIds` | направления, которыми внутри окна ограничивается список; по умолчанию — `directionId` категорий |

В режиме `booking: "lk"` витрина по первому нажатию «Записаться» создаёт оверлей
`#lk-overlay` и грузит бандл окна `/lk/group-schedule.js` (версия — из
`/lk/release.json`), затем монтирует его на выбранном `exerciseId`. Запись по
абонементу идёт через `POST /lk/subscription-bookings`, поэтому скидка, доплата
(например за 1/4 корта) и дневные лимиты считаются контуром, а не витриной.
Возврат из оплаты (`groupExerciseId` + `groupPaymentSuccess`/`groupPaymentFailed`)
витрина распознаёт сама, открывает окно на этом событии и убирает параметры.

Бандл окна приносит глобальные стили ЛК (`body`, `*`, `h1…h6`) — они переоформили
бы страницу-витрину, поэтому витрина держит оверлей на собственных стилях, а
глобальные стили окна подключает на время работы и снимает при закрытии.
...
| Опция | Значения |
| --- | --- |
| `cardsPerView` | число карточек в ряд (0 — фиксированная ширина; максимум 6). Действует только с 768px: на мобильном всегда одна карточка в кадре и свайп в сторону, её ширину задаёт `--atlanty-card-width` (её же переопределяет тема на брейкпоинтах) |
