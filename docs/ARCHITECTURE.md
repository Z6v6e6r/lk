...
`TournamentsPage` не вызывает API приставки напрямую. Кнопка в менеджере турнира отправляет пользовательский Bearer, `tournamentId` и `stationId` на SERV2. Для Сколково и Нагатинской при start дополнительно передаётся только логический target `right_arena`, `left_arena` или `both`; реальные device UUID не входят во frontend. Сколково показывает `Правый манеж / Левый манеж / Оба`, Нагатинская — `Экран Корт №1 / Экран Корт №7 / Оба экрана`. Node-RED проверяет профиль Viva, доступ к турниру и канонический station ID из сохранённого документа, разрешает `boxId` из серверной проекции настроек ЦУП, подставляет отдельный integration Bearer и только затем вызывает Android integration API.

Источник `stationId → tournamentBroadcastBoxId | tournamentBroadcastTargets` — настройки станции ЦУП. Для Node-RED они публикуются как `CUP_STATION_SETTINGS_JSON`; тестовые tournament/box override разрешены только отдельными server env и не входят в frontend или flow JSON. Перед multi-screen fan-out Node-RED атомарно сохраняет bounded `starting` claim с safe intended target keys; конкурентный start не обращается к устройствам, а финальная запись выполняется CAS по server-generated operation. Для `both` отправляются две device-команды, ответы объединяются до единственного finalize; любой неполный start компенсируется stop всех intended targets. Status читает фактическое состояние каждой приставки через box-control `GET /status`, сопоставляет `tournament_id` с текущим Viva ID и синхронизирует безопасное cached-состояние. `boxId`, integration Bearer и upstream URL не сохраняются и не возвращаются.
