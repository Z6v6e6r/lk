...
Клиент допускается только если его сертификат и заголовок `X-PadlHub-Client-Id`
указывают на одного и того же клиента:

```nginx
map $ssl_client_escaped_cert $pgm_v02_client { default ""; "~^<leaf>&" "<clientId>"; ... }
map "$ssl_client_verify:$pgm_v02_client" $pgm_v02_admitted { default 0; "SUCCESS:<clientId>" 1; ... }
map "$pgm_v02_client:$http_x_padlhub_client_id" $pgm_v02_client_bound { default 0; "<clientId>:<clientId>" 1; ... }
...
...
Имя переменной `$pgm_v02_client` историческое: на неё завязаны `limit_req_zone` и
`limit_conn_zone`, а nginx **отказывает в reload**, если ключ существующей зоны меняется
(`[emerg] ... uses the "..." key while previously it used the "..." key`). Переименование
потребовало бы полного рестарта общего ingress; меняются только значения map.

Добавление клиента — отдельное ревьюируемое изменение боевого контура:

1. добавить пару `<leaf>` → `<clientId>` и пару `<clientId>:<clientId>` в map-ы; один
   сертификат нельзя переиспользовать для второго `clientId`;
...
5. одновременно проверить, что `limit_req`/`limit_conn` покрывают ожидаемую нагрузку
   ...
   зоны считаются по идентичности из сертификата, а не по заголовку);
6. после reload убедиться по `generation` в audit-логе, что применился именно новый
   конфиг: молчаливый отказ reload не меняет воркеры, и старый `generation` — признак
   того, что на хосте всё ещё прежний конфиг.
...
```json
{
  ...
  "authorizedClients": { "<clientId>": ["<1–8 игр этого клиента>"] }
}
```
...
## 8. Выдача доступа партнёру рядом с канарейкой

Гвард принимает набор клиентов, поэтому партнёрский клиент добавляется **рядом** с
канареечным, а не вместо него. Порядок на каждое окружение:

1. keyring: добавить запись клиента (`enabled:true`, свои `scopes`, `stationIds`, `games`,
   `keys` с новым `keyId`); прошлые клиенты не трогать.
2. Анкор: добавить клиента в `authorizedClients` со списком **только его** игр. Keyring и
   анкор обязаны совпадать по составу, иначе старт будет отклонён.
3. Ingress: добавить пару лист→клиент и пару клиент:клиент в map-ы, добавить исходящие
   адреса партнёра в `allow`, `nginx -t`, `nginx -s reload`.
4. Проверить пред-полётом гвард на новом релизе (см. §5), затем `systemctl restart` и
   сквозную пробу **каждым** клиентом: своя пара сертификат+id должна доходить до
   приложения, чужая — получать `403` на ingress.

Ограничения, которые нельзя обходить:

- один лист нельзя выдать двум `clientId` — генератор это отклоняет;
- клиент не может получить игру вне своего набора в анкоре;
- `enabled:true` без объявления в анкоре — отказ старта (проверяется пред-полётом);
- канареечный клиент остаётся включённым только пока он нужен; при выводе из
  эксплуатации удалите его из keyring, из `authorizedClients` и из map-ов ingress, а
  также его `client.pem`/`client.key` с резервного хоста, и сделайте restart.

При выводе канареечного клиента следите за порядком: сначала keyring+анкор+restart
