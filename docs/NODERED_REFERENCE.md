...
- Managed annual status остаётся read-only, а purchase fail closed с `MANAGED_SUBSCRIPTION_SALE_READINESS_UNAVAILABLE`, пока нет authoritative sale-to-current-instance binding. Legacy-команда `prepare_tournament_subscription_sales_candidate.mjs` выведена из эксплуатации и после проверки provenance всегда завершается fail closed без записи candidate: текущие Piter-функции требуют атомарного изменения графа через `prepare_piter_atomic_sales_candidate.mjs`, а не только замены function bodies. Обнаруженный live-дубликат `Media2` сейчас disabled и не является активным route.
