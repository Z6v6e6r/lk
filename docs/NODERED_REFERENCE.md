...
- Зависший claim не освобождается request-путём: и release по отмене, и сверка просроченного `PENDING_CONFIRMATION` требуют нового запроса по тому же экземпляру абонемента. Поэтому поверх рантайма работает reconciler `npm run subscriptions:reconcile-hung-claims`: по TTL (`pendingUntil`/`leaseUntil`, иначе `updatedAt + --ttl-minutes`) он переводит не связанный с активной записью провайдера claim в `RELEASED` через compare-and-swap и только после полного readback Viva. Правила, причины отказа и порядок запуска: `docs/HUNG_SUBSCRIPTION_CLAIM_RECONCILE_20260912.md`.
...
- Managed annual status остаётся read-only, а purchase fail closed с `MANAGED_SUBSCRIPTION_SALE_READINESS_UNAVAILABLE`, пока нет authoritative sale-to-current-instance binding. Legacy-команда `prepare_tournament_subscription_sales_candidate.mjs` выведена из эксплуатации: после проверки provenance она всегда завершается fail closed без записи candidate и указывает на atomic replacement `prepare_piter_atomic_sales_candidate.mjs`. Повторное включение требует отдельного exact candidate binding и review; unified `BOUND` не разблокирует этот function-only builder. Обнаруженный live-дубликат `Media2` сейчас disabled и не является активным route.
...
- Реферальная атрибуция и шесть credential source-файлов ранее выпускались двухфазным `nodered:referral-attribution:audit` → reviewed contract SHA → `nodered:referral-attribution:candidate`. После перехода Piter sales на атомарный граф этот function-only candidate имеет состояние `UNBOUND_AFTER_PITER_ATOMIC_SALES`: audit и builder завершаются fail closed без артефактов. Повторное использование требует отдельного exact-graph rebind и review; обновление закреплённых SHA без графового контракта запрещено.
