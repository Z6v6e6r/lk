...
### Отдельная стартовая квота 50 из 100 (локальная подготовка 2026-09-05)

Явный флаг `--initial-batch-remaining 50` готовит стартовый остаток первой
партии 50, не создавая фиктивных оплат. Финансовые `paidCount`, `takenCount`
и `legacyPaymentRefs` сохраняют сверенные фактические значения. Отдельный
`quotaAdjustment = 50 - baseline.paidCount` участвует только в расчёте
коммерческого остатка, ценовой партии и проверке вместимости. При baseline 41
корректировка равна 9; новые резервирования уменьшают остаток, подтверждение
оплаты не уменьшает его повторно, неуспешная оплата освобождает резерв.
После следующих 50 продаж начинается вторая партия. Общая схема четырёх
партий по 100 не изменена. Baseline больше 50 блокирует эту настройку.

Такой activation packet имеет `formatVersion=2`, sentinel — `schemaVersion=2`.
Старый оператор отвергает версию packet; новый сохраняет поддержку packet и
sentinel V1 без корректировки. Понижение версии packet с сохранённой квотой
отклоняется даже при пересчитанном digest. Квота и версия связаны с CAS
резервирования, dispatch, provider-result, confirm, activate/deactivate и
проверкой результата записи. Это не разрешение менять квоту действующего ledger.

Подготовка требует candidate report с `launchQuotaSchemaVersion=2` и всех
прежних доказательств сверки/блокировок. Начальный candidate builder не является
обновлятором уже установленной atomic-топологии: повторный запуск поверх
существующих `piter_atomic_*` не заменяет reviewed update candidate из свежего
live flow. До отдельной публикации и активации эти изменения не открывают продажи.
Managed-ограничения использования остаются выключенными.

Отдельный focused updater установленного графа, точные source/candidate SHA и
границы будущей публикации описаны в [PITER_QUOTA_UPDATE_20260905.md](PITER_QUOTA_UPDATE_20260905.md).

Регрессионные проверки покрывают расчёт и условия CAS в локальных fixtures;
это не физический тест конкурентных записей Mongo и не production purchase proof.
