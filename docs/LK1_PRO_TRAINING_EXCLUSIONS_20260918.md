...
## Уточнение политики (2026-09-22)

Для групповой ПРО-тренировки сохраняется полная разовая цена и запрет скидки/бесплатного
первого события. Если у клиента есть доступный owned-пакет **«Энергия 5»** или
**«Энергия 25»** с остатком посещений, `/group` показывает кнопку списания посещения;
сервер проводит её по legacy visit-пути. РА, Академия, Дружба, «Энергия турниры»,
неизвестные варианты и пакеты с нулевым остатком остаются запрещены.

Виджет использует `getGroupScheduleOwnedPacks(...)`, а Node-RED — точный allowlist
`isProTrainingEnergyPack(...)` перед LK1-правилами и денежным readback. Guarded-генерация
для уже установленного PRO-флоу подготовлена в
`scripts/patch_live_lk1_pro_training_energy_exclusions_hotfix.mjs` и
`scripts/deploy_nodered_lk1_pro_training_energy_exclusions_147.sh`; она изменяет только
`booking.func` и `preview.func`.
