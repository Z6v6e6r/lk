...
- `scripts/patch_nodered_subscription_price_preview.mjs` — `proTrainingEmbedding(declared)`
  встраивает тот же модуль в canonical closure (по образцу `planRulesEmbedding`), имя
  добавлено в `PREVIEW_INJECTED_EXPORTS`/`PREVIEW_INJECTED_FUNCTIONS`, поэтому
  `assertCanonicalExports` и проверка «каждый `canonical.*` объявлен» покрывают гард.
...
- `npm run test:pro-training-exclusion` — **52 pass / 0 fail**.
...
- `node --experimental-strip-types --test scripts/tests/lk1PlanRulesPreview.test.mjs` — 23/23
  (композиция превью с новым встраиванием).
...
2. Focused-генерация `scripts/patch_live_lk1_pro_training_exclusions_hotfix.mjs` по образцу
   ...
   После неё обязательны новые `PREVIEW_CANONICAL_SOURCE_SHA256.booking` и
   `preimages.json`/`HUB_PREIMAGES` пины.
3. Guarded deploy-script `scripts/deploy_nodered_lk1_pro_training_exclusions_147.sh`
...
4. `PLAN_RULES_MODULE_SHA256` / `PLAN_RULES_CONFIG_FRAGMENT_SHA256`
   (`scripts/patch_live_lk1_plan_rules.mjs`) перепиновывать **не** нужно: `lk1PlanRules.mjs`,
   `lk1Config` и `lk1Quote` не менялись.
5. Базовая (не-HUB) генерация `scripts/patch_nodered_subscription_booking_flow.mjs` пишет
...
- **Превью и запись расходятся по форме ответа.** Превью отвечает 200 с пустым списком, а
  ...
  не доходит; при прямом вызове API он получит отказ с понятным кодом.
- **Имя как fallback.** Новое ПРО-направление, созданное в Viva после выверки списка,
  поймается только токеном «ПРО» в названии; направление с другим названием потребует
  расширить список id на обеих сторонах.
