...
партнёр, не Viva**. Название партнёра и технический владелец пока не указаны.
...
В P0-01 подтверждён только тип потребителя. Все восемь строк в целом остаются
...
| --- | --- | --- | --- |
| P0-01 | Потребитель — отдельный партнёр (тип подтверждён пользователем); отдельный M2M client, scopes add/remove/read и server-owned tenant/station/game allowlist; удаление только своего membership | Название партнёра, границы доступа, технический/бизнес-владелец и канал экстренной блокировки | Партнёр + владелец интеграции PadlHub |
...
1. Указать название отдельного партнёра и технического владельца с его стороны; определить,
...
- Тип потребителя подтверждён: отдельный партнёр, не Viva. Как называется партнёр
  и кто отвечает за его API-клиент?
