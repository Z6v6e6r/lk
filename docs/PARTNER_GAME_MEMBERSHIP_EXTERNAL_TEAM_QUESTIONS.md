...
При исходной сверке письменные ответы сторон не были обнаружены; если они уже
получены, нужно указать существующий документ, а не проводить согласование заново.
Последующее уточнение пользователя от 2026-09-06: потребитель API — **отдельный
партнёр rusPadelUp, не Viva**. Технический контакт назначен пользователем в этой
переписке; имя и личные контакты не переносятся в репозиторий/публичный kit.
Название партнёра не является выданным machine `clientId`; provisioning не выполнен.
Канал согласования подтверждён пользователем: **через пользователя этой задачи**.
Он передаёт rusPadelUp комплект/вопросы и возвращает ответы в эту переписку.
Прямая отправка партнёру со стороны агента не требуется и не выполняется.
Этот канал согласования не заменяет ещё не утверждённый канал экстренной блокировки.
...
Владельцы указаны **ролями**: технический контакт партнёра назначен пользователем,
остальные назначения и полномочия на согласование ещё требуют подтверждения.
В P0-01 подтверждены тип/название потребителя, назначение технического контакта
и канал согласования через пользователя.
Ответов самой внешней команды пока нет. Все восемь строк в целом остаются
`AWAITING_CONFIRMATION`; номер соответствует вопросу.

| ID | Зафиксировано в текущем контракте | Что требуется для согласования | Ответственные роли |
| --- | --- | --- | --- |
| P0-01 | Потребитель rusPadelUp и назначение технического контакта подтверждены пользователем; отдельный M2M client, scopes add/remove/read и server-owned tenant/station/game allowlist; удаление только своего membership | Границы доступа, бизнес-владелец, подтверждение полномочий ответственных и канал экстренной блокировки | rusPadelUp + владелец интеграции PadlHub |
...
### Адресаты после уточнения потребителя

- **Отдельный партнёр:** его клиент вызывает PadlHub API server-to-server. От него
  нужны идентификация и владельцы (P0-01), payment/ID semantics (P0-03/04), клиентские
  HMAC/mTLS/egress и результаты vectors (P0-05/06), retry matrix (P0-07).
- **Backend PadlHub и владелец Viva API:** Viva остаётся провайдером бронирований.
  Они подтверждают provider contract, технического клиента, token lifecycle,
  idempotency/cancellation и capacity (P0-02/08). HMAC-клиент партнёра не является
  Viva technical client; доступ и credentials Viva партнёру через этот API не выдаются.
- **Инфраструктура/security/эксплуатация PadlHub:** серверный mTLS/ingress, ограничения
  и отзыв доступа, audit/recovery и release evidence. Подтверждение партнёром своей
  части не закрывает эти внутренние проверки и не заменяет официальный ответ Viva.
...
1. Получить от назначенного технического контакта rusPadelUp ответы на черновик ниже;
   отдельно назначить на стороне PadlHub владельца официального ответа Viva.
   Название партнёра и технический контакт повторно не запрашивать.
2. Предоставить пользователю [офлайн-комплект](partner-game-membership-kit/README.md)
   и черновик ниже для пересылки rusPadelUp; ответы партнёра пользователь возвращает
   в эту задачу. Не запрашивать канал согласования повторно, не обращаться к партнёру
   напрямую. Факт передачи пользователю не означает отправку или ответ партнёра.
...
### Черновик первого запроса rusPadelUp

**DRAFT / NOT_SENT.** Текст для согласования, без адреса доставки и персональных
данных. Это не сообщение о готовом live endpoint и не передача доступа.
Предназначен для пересылки пользователем; подтверждение доставки пока не получено.

> Готовим подключение rusPadelUp к отдельному Partner API PadlHub. Ваш backend сможет
> добавлять внешнего игрока с отметкой внешней оплаты и удалять только созданный
> вашим integration client membership. С Viva работает backend PadlHub; доступ
> к Viva для вашего клиента не требуется. Сейчас доступен только офлайн-комплект
> из трёх файлов с пятью эталонными запросами; live-подключение ещё не открыто.
>
> Просим техническую команду ответить по пунктам; секреты, private keys и данные
> реальных игроков в ответ не включать:
>
> 1. **P0-01:** подтвердите ответственных за API-клиент, внешнюю оплату и экстренное
>    отключение; укажите нужные границы доступа и канал экстренной связи.
> 2. **P0-05/06:** какой backend/язык используете; поддерживаются ли HMAC-SHA256,
>    mTLS и отдельные test/production identities; есть ли стабильный egress IP/CIDR?
>    Как обеспечиваются хранение/отзыв ключей, синхронизация часов и случайный nonce?
>    Нужны описания возможностей, не значения credentials.
> 3. **P0-05:** проверьте все пять vectors своей реализацией и сообщите результаты
>    POST, retry, DELETE, GET и Unicode, язык/версию клиента и обнаруженные отличия.
> 4. **P0-03/04:** подтвердите стабильный externalPlayerId без переиспользования,
>    уникальный payment.reference на membership; что означает завершённая оплата,
>    как обрабатываются сумма/валюта, нулевая/частичная оплата и последующий возврат?
> 5. **P0-07:** пришлите retry matrix для 202/409/429/5xx/обрыва связи. На новой
>    попытке proof меняется, команда и Idempotency-Key сохраняются; UNKNOWN не
>    разрешает слепое повторное добавление или автоматический DELETE/возврат.
> 6. **P0-08:** какие ещё ваши процессы могут одновременно записывать игроков,
>    какие ограничения по ёмкости/времени входа нужны и кто разбирает конфликты?
>
> Provider contract, технического Viva client, idempotency/cancellation и серверную
> защиту PadlHub проверяет отдельно с владельцем Viva-интеграции. Ответы на этот
> запрос не означают автоматического открытия live-доступа.

Приложение к будущей отправке: [существующий офлайн-комплект](partner-game-membership-kit/README.md).
Запрос не заменяет полные вопросы P0 ниже; ответы ссылаются на их decision IDs.

## P0 — блокирует любой реальный вызов Viva

### 1. Кто является сторонами и владельцем данных?

- Потребитель подтверждён: отдельный партнёр rusPadelUp, не Viva; технический контакт
  назначен пользователем. Требуется подтвердить полномочия ответственных сторон.
