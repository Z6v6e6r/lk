...
Секрет — минимум 32 байта. Секрет передаётся вне API и никогда не попадает ни в request, ни во
flow-описание, ни в систему контроля версий. Сравнение подписи на сервере — constant-time.
