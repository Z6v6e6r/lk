...
export type MexicanoFirstRoundMode = "random" | "by_level" | "equal_pairs";
export type MexicanoStandingsSortMode = "point_diff" | "total_points";
...
export type MexicanoClassicOptions = {
  ...
  tableSortMode?: MexicanoStandingsSortMode;
  winnerSortMode?: MexicanoStandingsSortMode;
  seed?: string | number;
...
type MexicanoPayloadLike = {
  tournamentType?: unknown;
  params?: unknown;
  rounds?: MexicanoRoundLike[] | null;
};

export type ClassicMexicanoRoundSaveResult = {
  roundId: string;
  matchId: string;
  court?: string;
  courtIndex?: number;
...
function normalizeParticipantRef(value: unknown) {
  if (value && typeof value === "object") {
    const record = value as { id?: unknown; phone?: unknown };
    return normalizeText(record.id ?? record.phone);
  }
  return normalizeText(value);
}

function isClassicMexicanoType(value: unknown) {
  const normalized = normalizeText(value).toLowerCase();
  return normalized === "mexicano" || normalized === "classic_mexicano" || normalized === "mexicano_classic";
}

function isParticipantRefList(value: unknown) {
  const ids = Array.isArray(value) ? value.map(normalizeParticipantRef).filter(Boolean) : [];
  return ids.length === 2 && new Set(ids).size === 2;
}

export function isClassicMexicanoMatchLayoutComplete(
  match: MexicanoRoundLike["matches"] extends Array<infer T> | null | undefined ? T : never,
) {
  if (!match || typeof match !== "object") return false;
  const hasCourt =
    normalizeText(match.court).length > 0
    || (typeof match.courtIndex === "number" && Number.isFinite(match.courtIndex));
  return hasCourt && isParticipantRefList(match.pair1) && isParticipantRefList(match.pair2);
}

export function isClassicMexicanoRoundLayoutComplete(round: MexicanoRoundLike | null | undefined) {
  const matches = Array.isArray(round?.matches) ? round.matches : [];
  return matches.length > 0 && matches.every(isClassicMexicanoMatchLayoutComplete);
}

export function isClassicMexicanoRoundValidlyCompleted(round: MexicanoRoundLike | null | undefined) {
  const matches = Array.isArray(round?.matches) ? round.matches : [];
  return (
    isClassicMexicanoRoundLayoutComplete(round)
    && matches.every((match) => match.score1 != null && match.score2 != null)
  );
}

function getClassicMexicanoRoundStructuralScore(round: MexicanoRoundLike | null | undefined) {
  const matches = Array.isArray(round?.matches) ? round.matches : [];
  return matches.reduce((score, match) => {
    const hasScore = match.score1 != null && match.score2 != null;
    const hasLayout = isClassicMexicanoMatchLayoutComplete(match);
    return score + (hasLayout ? 10 : 0) + (hasLayout && hasScore ? 3 : 0) + (hasScore ? 1 : 0);
  }, 0);
}

export function getClassicMexicanoPayloadStructuralScore(payload: MexicanoPayloadLike | null | undefined) {
  if (!payload || !isClassicMexicanoType(payload.tournamentType)) return 0;
  const rounds = Array.isArray(payload.rounds) ? payload.rounds : [];
  return rounds.reduce((score, round) => score + getClassicMexicanoRoundStructuralScore(round), 0);
}

export function shouldPreferClassicMexicanoSnapshot(
  candidate: MexicanoPayloadLike | null | undefined,
  current: MexicanoPayloadLike | null | undefined,
  options?: { hasPendingSync?: boolean },
) {
  if (!candidate) return false;
  if (!isClassicMexicanoType(candidate.tournamentType)) return true;
  if (!current || !isClassicMexicanoType(current.tournamentType)) return true;
  if (options?.hasPendingSync) return true;
  return getClassicMexicanoPayloadStructuralScore(candidate) >= getClassicMexicanoPayloadStructuralScore(current);
}

export function shouldPreferClassicMexicanoCachedSnapshot(
  candidate: MexicanoPayloadLike | null | undefined,
  current: MexicanoPayloadLike | null | undefined,
  options?: {
  ...
  if (!candidate) return false;
  const candidateIsNotStructurallyWorse = shouldPreferClassicMexicanoSnapshot(candidate, current, {
    hasPendingSync: options?.hasPendingSync,
  });
  if (!candidateIsNotStructurallyWorse) return false;
  if (!current) return true;
  if (options?.hasPendingSync) return true;

  const currentIsNotStructurallyWorse = shouldPreferClassicMexicanoSnapshot(current, candidate);
  const candidateUpdatedAt = options?.candidateUpdatedAt ?? 0;
  const currentUpdatedAt = options?.currentUpdatedAt ?? 0;
  return candidateUpdatedAt >= currentUpdatedAt || !currentIsNotStructurallyWorse;
}

export function shouldSeedClassicMexicanoServerRoundLayout(
  currentRound: AmericanoLabRound | null | undefined,
  persistedRoundBeforeSave: MexicanoRoundLike | null | undefined,
) {
  if (!currentRound) return false;
  const persistedMatches = Array.isArray(persistedRoundBeforeSave?.matches)
    ? persistedRoundBeforeSave.matches
    : [];
  return (
    !persistedRoundBeforeSave
    || persistedMatches.length < currentRound.matches.length
    || !isClassicMexicanoRoundValidlyCompleted(persistedRoundBeforeSave)
  );
}

export function buildClassicMexicanoRoundSaveResults(
  round: AmericanoLabRound,
  matchId: string,
  persistedRoundBeforeSave?: MexicanoRoundLike | null,
): ClassicMexicanoRoundSaveResult[] {
  if (shouldSeedClassicMexicanoServerRoundLayout(round, persistedRoundBeforeSave)) {
    return round.matches.map((roundMatch) => {
      const shouldPersistScore = roundMatch.id === matchId || roundMatch.saved;
      const payload: ClassicMexicanoRoundSaveResult = {
        roundId: round.id,
        matchId: roundMatch.id,
        court: roundMatch.court,
        courtIndex: roundMatch.courtIndex,
        pair1: roundMatch.pair1.map((player) => player.id),
        pair2: roundMatch.pair2.map((player) => player.id),
      };
  ...
  const match = round.matches.find((roundMatch) => roundMatch.id === matchId);
...
function buildClassicMexicanoRoundLayoutResults(
  round: AmericanoLabRound,
): ClassicMexicanoRoundSaveResult[] {
...
export function buildClassicMexicanoMatchSaveResults(
  participants: AmericanoLabParticipant[],
  courts: string[],
  rounds: AmericanoLabRound[],
  roundId: string,
  matchId: string,
  persistedRoundsBeforeSave: MexicanoRoundLike[] | null | undefined,
  options?: MexicanoClassicOptions,
): ClassicMexicanoRoundSaveResult[] {
  const currentRound = rounds.find((round) => round.id === roundId);
  if (!currentRound) return [];

  const persistedRounds = Array.isArray(persistedRoundsBeforeSave)
    ? persistedRoundsBeforeSave
    : [];
  const persistedRound = persistedRounds.find((round) => normalizeText(round?.id) === roundId);
  ...
  if (isClassicMexicanoRoundValidlyCompleted(persistedRound)) {
    return results;
  }

  const savedScoreByMatchKey = new Map(
    results
      .filter((result) => result.score1 != null && result.score2 != null)
      .map((result) => [`${result.roundId}::${result.matchId}`, result] as const),
  );
  const roundsAfterCurrentSave = rounds.map((round) => {
    if (round.id !== roundId) return round;
    const matches = round.matches.map((match) => {
      const savedResult = savedScoreByMatchKey.get(`${round.id}::${match.id}`);
      if (!savedResult) return match;
      return {
        ...match,
        score1: savedResult.score1 ?? match.score1,
        score2: savedResult.score2 ?? match.score2,
        saved: true,
      };
    });
    return {
  ...
  const savedCurrentRound = roundsAfterCurrentSave.find((round) => round.id === roundId);
  if (!savedCurrentRound?.saved) {
    return results;
  }

  const roundsWithNext = rebuildMexicanoClassicFutureRounds(
    participants,
    courts,
    roundsAfterCurrentSave,
    savedCurrentRound.index,
    options,
  );
  const nextRound = roundsWithNext.find((round) => round.index === savedCurrentRound.index + 1);
  if (!nextRound) {
    return results;
  }

  const persistedNextRound = persistedRounds.find(
    (round) => normalizeText(round?.id) === nextRound.id,
  );
  if (isClassicMexicanoRoundLayoutComplete(persistedNextRound)) {
    return results;
  }

  return [
    ...results,
    ...buildClassicMexicanoRoundLayoutResults(nextRound),
  ];
}
...
function rankPlayersByTable(
  ...
  sortMode: MexicanoStandingsSortMode,
) {
...
function pickBestPairingForGroup(
  ...
  context: MexicanoHistoryContext,
  weights: MexicanoPenaltyWeights,
  forceEqualPairs = false,
) {
  if (forceEqualPairs) {
    return {
      ...evaluatePairingCandidate(
        group,
        [[0, 3], [1, 2]],
        "balance_14_vs_23",
        "1+4 vs 2+3",
...
function buildInitialRanking(
  ...
  const ordered = firstRoundMode === "by_level" || firstRoundMode === "equal_pairs"
...
function buildMexicanoRoundFromRanking(
  ...
  const drafts = optimizedGroups.map((group) => pickBestPairingForGroup(
    group,
    context,
    weights,
    firstRound && firstRoundMode === "equal_pairs",
  ));
    ...
    firstRound
      ? `стартовый раунд (${
        firstRoundMode === "random"
          ? "детерминированная жеребьевка"
          : firstRoundMode === "equal_pairs"
            ? "посев по уровню + равные пары (1+4 vs 2+3)"
            : "посев по уровню"
      })`
      : "раунд по актуальной таблице",
    firstRound && firstRoundMode === "equal_pairs"
      ? "схема внутри четверки: 1+4 vs 2+3"
...
function resolveOptions(options?: MexicanoClassicOptions) {
  return {
    ...
    firstRoundMode: options?.firstRoundMode ?? "by_level",
    byeMode: options?.byeMode ?? "rotating_bye",
    tableSortMode: options?.tableSortMode ?? "total_points",
    winnerSortMode: options?.winnerSortMode ?? "point_diff",
    seed: options?.seed,
...
function buildNextRound(
  ...
  const ranking = completedRounds.length === 0
    ...
    : rankPlayersByTable(players, context, resolved.seed, resolved.tableSortMode);
...
export function buildMexicanoClassicParams(
  ...
  return {
    ...
    tableSortMode: resolved.tableSortMode,
    winnerSortMode: resolved.winnerSortMode,
