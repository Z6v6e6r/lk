...
export default function GamesPage({
  ...
  const upsertGameRecordInStores = useCallback((
    incoming: PadelGameRecord,
    options?: {
      communityMode?: "upsert" | "if_exists" | "skip";
      recordMode?: "merge" | "replace";
    },
  ) => {
    const communityMode = options?.communityMode ?? "if_exists";
    const replaceRecord = options?.recordMode === "replace";
    const recordForStore = replaceRecord
      ? mergePadelGameRecord(null, incoming)
      : incoming;
    if (communityMode !== "skip") {
      setCommunityGames((prev) => {
        ...
        if (!replaceRecord) return upsertPadelGameRecord(prev, incoming);
        const existingIndex = prev.findIndex((item) => item.id === incoming.id);
        if (existingIndex < 0) return [recordForStore, ...prev];
        const next = [...prev];
        next[existingIndex] = recordForStore;
        return next;
      });
    }
    setActiveGameRecordStore((prev) => {
      ...
      if (activeId && incoming.id === activeId) {
        return replaceRecord
          ? recordForStore
          : (prev ? mergePadelGameRecord(prev, incoming) : incoming);
      }
      if (prev && prev.id === incoming.id) {
        return replaceRecord ? recordForStore : mergePadelGameRecord(prev, incoming);
      }
      return prev;
    });
    notifyGameRecordsUpdated(
      [recordForStore],
      replaceRecord ? "games_replace_record" : "games_upsert_record",
    );
  }, [gameRecordId]);
  ...
  const detailsSourceParticipants = useMemo<PadelGamePlayer[]>(() => (
    Array.isArray(activeGameRecord?.participants)
      ? activeGameRecord.participants
      : participants
  ...
  const detailsWaitlist = useMemo<PadelGamePlayer[]>(() => {
    const sourceWaitlist =
      Array.isArray(activeGameRecord?.waitlist)
  ...
  const detailsJoinPricePreview = useSubscriptionPricePreview({
    target: createJoinSubscriptionPriceTarget({ gameId: gameRecordId,
  ...
  useEffect(() => {
    ...
    void apiFetchTournamentParticipants(exerciseId, { sanitize: false })
      .then((result) => {
        ...
        void apiUpdatePadelGameRecord(activeGameRecord.id, patchPayload).then(async (patchResult) => {
          if (!alive) return;
          ...
          const authoritative = await apiFetchPadelGameRecord(activeGameRecord.id);
          if (!alive) return;
          if (authoritative.data?.id) {
            upsertGameRecordInStores(
              authoritative.data as PadelGameRecord,
              { communityMode: "if_exists", recordMode: "replace" },
            );
          }
          trackRosterSyncEvent("failed", {
            ...
            restoredPlayersCount,
            durationMs,
            reason: authoritative.data?.id
              ? "patch_rejected_authoritative_refreshed"
              : "patch_rejected_refresh_failed",
  ...
  const handleSplitGamePay = useCallback(async (
    ...
    const resolvedClientSubscriptionId =
      resolvedPaymentMode === "subscription"
        ...
        : null;
    if (resolvedPaymentMode === "subscription" && !resolvedClientSubscriptionId) {
    ...
    if (paymentResult.error || !paymentResult.data) {
      ...
      if (dailyLimitMessage) {
        ...
        setPayError(formatSubscriptionDecisionPresentation(
          resolveSubscriptionDecisionPresentation({
            action: "CREATE_GAME",
  ...
  const submitSplitGamePayment = useCallback(async (
    preferredPaymentMode?: "subscription" | "one_time",
    preferredClientSubscriptionId?: string | null,
  ) => {
    const guarded = preferredPaymentMode === "subscription";
    if (guarded && splitSubscriptionSubmitInFlightRef.current) return;
    if (guarded) splitSubscriptionSubmitInFlightRef.current = true;
    try {
      await handleSplitGamePay(preferredPaymentMode, preferredClientSubscriptionId);
      ...
      if (guarded) {
        ...
        setPayError("Временная техническая ошибка. Повторите попытку.");
      }
      setLoadingPay(false);
    } finally {
      if (guarded) splitSubscriptionSubmitInFlightRef.current = false;
    }
  }, [duration, handleSplitGamePay]);
  ...
  const handleCreateSubmit = useCallback(() => {
    ...
    if (splitPaymentSelected) {
      if (splitCheckoutMode === "subscription" && splitHasSubscriptionPaymentOptions) {
        ...
        void submitSplitGamePayment("subscription", selectedSplitSubscriptionId);
        return;
      }
      void submitSplitGamePayment("one_time");
      return;
    }
  ...
  const handleLeaveCurrentUserFromDetails = useCallback(async () => {
    ...
    if (!isSelfLeavePreviewMode) {
      ...
      if (refreshed.error || !refreshedRecord || hasActiveGameLeaveMembership(refreshedRecord, {
        ...
        setUpdatingGameRoster(false);
        setLeavePendingMessage(null);
        setGameRosterError(refreshed.error?.message || lastPendingMessage
          || "Выход ещё не подтверждён. Повторите выход, чтобы завершить удаление записи участия.");
        return;
      }
  ...
  const handleSplitJoinCurrentUserFromDetails = useCallback(async (
    ...
    if (preferredPaymentMode === "subscription"
      && detailsJoinPricePreview.bySubscriptionId[String(preferredClientSubscriptionId || "")]?.state !== "available") {
      setGameRosterError("Сначала проверьте стоимость и лимиты по выбранной подписке");
      return;
    }
    ...
    if (preferredPaymentMode === "subscription" && detailsSubscriptionSubmitInFlightRef.current) return;
    ...
    try {
      ...
      if (preferredPaymentMode === "subscription") {
        detailsSubscriptionSubmitInFlightRef.current = false;
      }
    }
  }, [
    detailsJoinPricePreview.bySubscriptionId,
                ...
                detailsCurrentUserPendingSplitPayment && !detailsCurrentUserPendingSplitPaymentIsExpired ? (
                  ...
                  <div className="game-join-split-pay-actions">
                    ...
                    {!detailsSplitSubscriptionsLoading
                      && !detailsSplitSubscriptionsError
                      && detailsSplitSubscriptionOptions.length > 0 && (
                      <JoinSubscriptionOptions options={detailsSplitSubscriptionOptions} preview={detailsJoinPricePreview}
                        disabled={!canCurrentUserJoinSplitGameInDetails} shareLabel={`1/${detailsSplitShareCount}`}
                        onJoin={id => { void handleSplitJoinCurrentUserFromDetails("subscription", id); }} />
                    )}
                    <button
                      className="game-join-split-pay-option game-join-split-pay-option-primary"
                      type="button"
          ...
          <div className="game-payment-stack">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (splitPaymentSelected) {
                  void submitSplitGamePayment(splitHasEligibleSubscriptions ? "one_time" : undefined);
                } else {
                  void handleMasterServicePay();
                }
              }}
