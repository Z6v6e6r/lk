...
function resolveTournamentResultsSortMode(
  ...
  const explicitPointsFlag =
    toTruthy(params?.sortByTotalPoints)
    || toTruthy(params?.useTotalPointsRanking)
    || toTruthy(summary?.sortByTotalPoints)
    || toTruthy(summary?.useTotalPointsRanking);
  ...
  const hasPointsMode = modeCandidates.some((value) => (
    value === "points"
    || value === "total_points"
    || value === "totalpoints"
    || value === "points_for"
    || value === "pointsfor"
    || value === "набранные_очки"
    || value === "набранныеочки"
  ));
  if (hasPointsMode) return "total_points";
