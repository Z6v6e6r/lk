...
interface CommunityTournamentCardProps {
  ...
  onOpen: () => void;
  onRequestPriceDetails?: () => void;
  variant?: "default" | "groupSchedule";
}

const FRIENDLY_TAG_LABEL = "Лето.Падел";
const FRIENDLY_TAG_SUBSCRIPTION_URL = "https://padlhub.ru/ab_leto";
const FRIENDLY_TAG_MODAL_CTA_LABEL = "Оформить подписку";
...
export function CommunityTournamentCard({
  card,
  onOpen,
  onRequestPriceDetails,
  variant = "default",
}: CommunityTournamentCardProps) {
  ...
  const isGroupScheduleVariant = variant === "groupSchedule";
  ...
  const pricePopover = card.pricePopover ?? null;
  const summerSubscriptionOffer = card.summerSubscriptionOffer ?? null;
  const priceLabel = pricePopover?.triggerLabel?.trim() || card.priceLabel?.trim() || "энергия";
  const hasPricePopoverRows = (pricePopover?.rows.length ?? 0) > 0;
  const canOpenEnergyPopover =
    Boolean(onRequestPriceDetails)
    || Boolean(card.pricePopoverLoading)
    || hasPricePopoverRows
    || Boolean(summerSubscriptionOffer);
  const waitlistCount = Math.max(0, card.waitlistCount ?? 0);
  const shouldShowWaitlist = !isGroupScheduleVariant || waitlistCount > 0;
  ...
  const showFriendlySubscriptionTag = Boolean(card.hasFriendlySubscriptionTag) && spotsLeft > 0;
  ...
  return (
    <CommunityFeedCardBase variant="tournament" className={styles.card}>
      <div className={styles.layout}>
        ...
        <div className={`${styles.tournament}${isGroupScheduleVariant ? ` ${styles.tournamentGroupSchedule}` : ""}`}>
          <div className={styles.topLine}>
            {!isGroupScheduleVariant && (
          ...
          {showFriendlySubscriptionTag && (
            <div className={styles.friendlyTagRow}>
              <button
                type="button"
                className="find-game-friendly-tag"
                aria-label="Турнир доступен по подписке Лето.Падел"
                onClick={(event) => {
          ...
          <div className={styles.priceWrap} ref={energyPriceRef}>
            ...
            {canOpenEnergyPopover && isEnergyPopoverOpen && (
              ...
              >
                <div className={styles.energyPopoverTitle}>Доступные услуги</div>
                {card.pricePopoverLoading ? (
                  <div className={styles.energyPopoverEmpty}>Проверяем доступные варианты...</div>
                ) : hasPricePopoverRows || summerSubscriptionOffer ? (
                  <>
                    {pricePopover?.rows.map((row) => (
                      <div className={styles.energyPopoverRow} key={row.id}>
                        <span>{row.label}</span>
                    ...
                    {summerSubscriptionOffer ? (
                      <>
                        {hasPricePopoverRows ? <div className={styles.energyPopoverDivider} /> : null}
                        <button
                          type="button"
                          className={`${styles.energyPopoverPromoButton} ${styles.energyPopoverRow} ${styles.energyPopoverRowPromo}`}
                          onClick={() => {
                            navigateToExternalUrl(FRIENDLY_TAG_SUBSCRIPTION_URL);
                          }}
                          aria-label={`Приобрести ${summerSubscriptionOffer.label} за ${summerSubscriptionOffer.value}`}
                        >
                          <span>{summerSubscriptionOffer.label}</span>
                          <strong>{summerSubscriptionOffer.value}</strong>
                        </button>
                      </>
                    ) : null}
                  </>
                ) : (
                  <div className={styles.energyPopoverEmpty}>Подходящие услуги не найдены</div>
                )}
              </div>
            )}
          </div>
            ...
            <div className={styles.capacityLabels}>
              ...
              {!isGroupScheduleVariant && (
                <div className={styles.capacityRemaining}>
                  {`осталось: ${spotsLeft} ${spotsLeft === 1 ? "место" : spotsLeft >= 2 && spotsLeft <= 4 ? "места" : "мест"}`}
                </div>
              )}
            </div>
          </div>
      ...
      >
        <div className="find-game-friendly-modal">
          <p className="find-game-friendly-modal-text">Записаться можно по подписке</p>
          <button
            type="button"
