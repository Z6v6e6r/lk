...
const RETRYABLE_PUSH_HTTP_STATUSES = new Set([404, 405, 429, 500, 502, 503, 504]);
...
function getAuthToken() {
  return normalizeToken(readAuthToken());
}
...
function resolvePushEndpointCandidates(endpointRaw: string | null | undefined) {
  const endpoint = String(endpointRaw || "").trim();
  if (!endpoint) return [];
  return buildProjectUrlCandidates(endpoint, SERV2, SERV2_FALLBACK);
}

async function postPushPayload(
  ...
  for (const endpoint of endpointCandidates) {
    ...
    const endpointError = new Error(`Push ${action} request failed: HTTP ${response.status}`);
    if (!RETRYABLE_PUSH_HTTP_STATUSES.has(response.status)) {
      throw endpointError;
    }
    lastError = endpointError;
  }

  throw lastError ?? new Error(`Push ${action} request failed`);
}
...
export async function syncPushTokenWithBackend(reason: PushSyncReason = "manual") {
  ...
  const endpointCandidates = resolvePushEndpointCandidates(PUSH_REGISTRATION_URL);
  if (endpointCandidates.length === 0) return;
  ...
  state.syncInFlight = (async () => {
    try {
      await postPushPayload(endpointCandidates, authToken, buildRegistrationPayload(token), "register");
      ...
      trackClientError(
        "push.sync_failed",
        error,
        { reason, endpoints: endpointCandidates },
        { handled: true, severity: "warning" },
      );
    } finally {
...
export async function unregisterPushToken(reason: PushSyncReason = "logout") {
  ...
  const token = normalizeToken(state.token ?? readStorage(PUSH_TOKEN_STORAGE_KEY));
  const authToken = getAuthToken();
  const endpointCandidates = resolvePushEndpointCandidates(PUSH_UNREGISTRATION_URL);

  if (token && authToken && endpointCandidates.length > 0) {
    try {
      await postPushPayload(
        endpointCandidates,
        authToken,
        {
          token,
