...
if [[ $# -ne 1 || "${NODE_RED_MANAGED_SUBSCRIPTION_RULES_ROLLBACK:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_MANAGED_SUBSCRIPTION_RULES_ROLLBACK=CONFIRM_147 npm run nodered:managed-subscription-rules:rollback-147 -- YYYYMMDDTHHMMSS+ZZZZ" >&2
  exit 2
fi
