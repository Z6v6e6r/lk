...
# Guarded deployment of the focused LK1 "event already started" generation.
#
# One node, one field: lk_subscription_price_preview_20260908_router.func. The twelve target
# conditions of the event route are named, the refusal union is unchanged, and a target that
# is healthy in every respect except that it already started answers 200 with no quotes
# instead of a fail-closed 503. Measured on production on 2026-09-17, every refusal of the
# chronic `*_TARGET_UNRESOLVED` class (10-22 a day) was exactly that state.
#
# The patcher proves the refusal call sites are unchanged (count and code expression, 44) and
# that the started branch is present; scripts/tests/lk1TargetDiagnostics.test.mjs enumerates
# all 4096 combinations of the twelve conditions and asserts the decision equals the original
# conjunction.
#
# The candidate changes exactly one `func` field of one node; every other node, the booking
# gateway, the evaluator and the `subscriptions_lk1_plan_rules` activation stay byte-identical.
# No node is added and no route changes.
#
# # DEVIATION FROM THE OTHER GENERATION WRAPPERS: as with `lk1-target-diagnostics`, the
# checkout does not have to sit on `main`. The reviewed source is pinned by commit
# (`generation_commit` below, `git merge-base --is-ancestor`), the generation's own sources
# must be unchanged since that commit, HEAD must be published on an origin branch and the tree
# must be clean.
#
# Requires an explicit confirmation variable and the exact live preimage. Everything else
# fails closed.

set -euo pipefail
umask 077

if [[ $# -ne 0 || "${NODE_RED_LK1_EVENT_STARTED_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_EVENT_STARTED_DEPLOY=CONFIRM_147 npm run nodered:lk1-event-started:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-event-started-unavailable"
allow_nodes=(lk_subscription_price_preview_20260908_router)
...
expected_node_fields='{"lk_subscription_price_preview_20260908_router":["func"]}'
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-event-started-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1esu.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_EVENT_STARTED_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_event_started_unavailable_hotfix.mjs \
  --workspace "$workspace" \
