...
const parseRequestJson = (value, label, errors) => {
  const text = toStr(value);
  if (!text) return null;
  try {
...
const buildCompositeCandidate = (rawSegments, defaults = {}) => {
  ...
  return {
    ok: true,
    value: {
      id: `${pattern.key}:${segments.map((segment) => segment.slotId).join(">")}`,
      targetDurationMinutes: pattern.targetDurationMinutes,
      patternKey: pattern.key,
      patternLabel: pattern.label,
      fromTime: first.timeFrom,
      toTime: segments[segments.length - 1].timeTo,
      transitionCount,
...
const requestDefaults = {
  stationId: toStr(body.stationId || body.stationUUID || body.stationUuid || query.stationId || query.stationUUID || query.stationUuid),
  studioId: toStr(body.studioId || body.clubId || body.clubUUID || body.studioUuid || query.studioId || query.clubId || query.clubUUID || query.studioUuid),
  date: normalizeDate(body.date || body.day || body.bookingDate || query.date || query.day || query.bookingDate),
};

const querySegments = parseRequestJson(query.segments, "segments", parseErrors);
const querySlots = parseRequestJson(query.slots, "slots", parseErrors);
if (parseErrors.length > 0) {
  return fail(400, "Invalid query JSON", parseErrors);
}

const rawSegments = asArray(body.segments).length > 0
  ? body.segments
  : (asArray(body.compositeBooking && body.compositeBooking.segments).length > 0
    ? body.compositeBooking.segments
    : querySegments);

if (asArray(rawSegments).length > 0) {
  const built = buildCompositeCandidate(rawSegments, requestDefaults);
  if (!built.ok) return fail(400, built.error.message, built.error.details || built.error);
  return ok({
    ok: true,
    mode: "segments",
    contractVersion: 1,
    candidates: [built.value],
    validation: {
...
const rawSlots = asArray(body.slots).length > 0 ? body.slots : querySlots;
if (!Array.isArray(rawSlots) || rawSlots.length === 0) {
  return fail(400, "segments or slots array is required");
}

const normalizedSlots = rawSlots.map((slot, index) => normalizeSegment(slot, index, requestDefaults));
const slotErrors = normalizedSlots.filter((item) => item.ok === false).map((item) => item.error);
if (slotErrors.length > 0) {
  return fail(400, "slots contain invalid descriptors", slotErrors);
}
...
const pushCandidate = (segments) => {
  const built = buildCompositeCandidate(segments, requestDefaults);
  if (!built.ok) return;
  const dedupeKey = built.value && built.value.compositeBooking ? built.value.compositeBooking.dedupeKey : null;
  if (!dedupeKey || seenDedupeKeys.has(dedupeKey)) return;
  seenDedupeKeys.add(dedupeKey);
  candidates.push(built.value);
};

for (let index = 0; index < slots.length; index += 1) {
  pushCandidate([slots[index]]);
  for (let nextIndex = index + 1; nextIndex < slots.length; nextIndex += 1) {
    pushCandidate([slots[index], slots[nextIndex]]);
  }
}

candidates.sort((left, right) => {
  const startDelta = Date.parse(left.compositeBooking.startAt) - Date.parse(right.compositeBooking.startAt);
  if (startDelta !== 0) return startDelta;
  return left.compositeBooking.segmentCount - right.compositeBooking.segmentCount;
});
