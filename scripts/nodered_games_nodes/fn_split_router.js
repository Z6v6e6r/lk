...
const pickBestSubscriptionProduct = (products) => {
...
if (ctx.step === "available_products") {
  ...
  const subscriptionProduct = pickBestSubscriptionProduct(products);
  ...
  ctx.subscriptionProductId = subscriptionProduct?.id || null;
  ctx.subscriptionProductName = subscriptionProduct?.name || null;
  ...
  const transactionPayload = {
    ...
    discountReason: selectedMode === "subscription"
      ? "Открытая игра 4/4: списание посещения абонемента"
