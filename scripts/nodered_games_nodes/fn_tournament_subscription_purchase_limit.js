...
if (!subscriptionCounterEpoch.admission(ctx, global)) return failMsg(503, "Продажи этого периода закрыты", { code: "COUNTER_EPOCH_ADMISSION_CLOSED" });
if (subscriptionCounterEpoch.isNew(ctx.counterKey, ctx.inventoryId) && rows.some(row => row.paymentRef === ctx.paymentRef && row.inventoryId !== ctx.inventoryId)) {
  return failMsg(409, "Платёжная ссылка относится к предыдущему периоду", { code: "COUNTER_EPOCH_PAYMENT_REF_CONFLICT" });
}
const now = Date.now();
...
if (managedSaleCounterKey === "network_friendship"
  ...
  return failMsg(503, "Правила продажи ХАБ изменились", { code: "HUB_LK1_SALE_BINDING_DRIFT" });
}
ctx.vivaTokenRequestBody = vivaTokenRequestBody;

if (managedSaleCounterKey === "network_friendship" && !hubLk1Sale) {
