...
const previousSummary = tournament.summary && typeof tournament.summary === "object"
  ? tournament.summary
  : {};
const tournamentFinished = isTournamentMarkedFinished(params, previousSummary);
if (tournamentFinished) {
  ...
  params.completedAt = String(params.completedAt || finishedAt);
}
...
const applyIncomingResults = () => {
  ...
  results.forEach((result, resultIndex) => {
    ...
    if (hasIncomingPair1 || hasIncomingPair2) {
      update.layout = {
        pair1: normalizeIdArray(result.pair1, `${round.id || "round"}-${match.id || "match"}-pair1`),
        pair2: normalizeIdArray(result.pair2, `${round.id || "round"}-${match.id || "match"}-pair2`),
        court: result.court,
    ...
    if (!updatesByRoundId.has(round.id)) updatesByRoundId.set(round.id, []);
    updatesByRoundId.get(round.id).push(update);
  });

  updatesByRoundId.forEach((updates, roundId) => {
    const round = updates[0]?.round;
    if (!round) return;
    ...
    if (layoutUpdates.length === 0) {
      updates.forEach((update) => {
        ...
        if (result?.court) update.match.court = result.court;
        if (result?.courtIndex !== undefined) update.match.courtIndex = result.courtIndex;
      });
      return;
    }

    layoutUpdates.forEach((update) => {
      const current = candidateLayouts.get(update.match.id) || {
        pair1: [],
        pair2: [],
        court: update.match.court,
        courtIndex: update.match.courtIndex,
      };
      candidateLayouts.set(update.match.id, {
    ...
    if (!validateRoundLayouts(candidateLayouts)) return;

    layoutUpdates.forEach((update) => {
      const nextLayout = candidateLayouts.get(update.match.id);
      if (!nextLayout) return;
      update.match.pair1 = nextLayout.pair1;
      update.match.pair2 = nextLayout.pair2;
      if (nextLayout.court) update.match.court = nextLayout.court;
      if (nextLayout.courtIndex !== undefined) update.match.courtIndex = nextLayout.courtIndex;
    });
  });
};
...
maybeAppendClassicMexicanoRound();
...
const summary = {
  ...
  completedMatches,
  byePolicy: BYE_POINTS_MODE === "zero_points" ? BYE_POLICY_ZERO : BYE_POLICY,
  status: tournamentFinished || allScheduledMatchesCompleted ? "completed" : "in_progress",
