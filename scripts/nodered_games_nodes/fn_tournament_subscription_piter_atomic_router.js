...
if (ctx.step === "piter_replay_sale_ack") {
  // The ledger's cached response cannot tell whether the checkout was archived.
...
if (ctx.step === "piter_replay_sale_readback") {
  ...
  return response(ctx.saleResponseStatus || 200, paymentPolling.response(ctx.providerResult?.response || { ok: true, replayed: true }, record));
}
