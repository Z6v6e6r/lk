...
# Guarded deployment of the focused LK1 event tariff-amounts generation.
#
#   * lk_subscription_price_preview_20260908_router.func — the event tariff treats the
#     paid price (`cost`/`price`/`amount`) as the authoritative amount and only requires
#     the trial price (`trialCost`) to be a non-negative integer;
#   * lk_subscription_price_preview_20260908_evaluate.func — the preview evaluator is
#     re-bound to the reviewed body the write path already runs, so the stale body that
#     compared the rule product with the hardcoded HUB id and still emitted the removed
#     cap blocker is gone (live diagnostics: LK1_PRODUCT_BINDING_INVALID +
#     ACTIVE_SERVICES_LIMIT_REACHED on event quotes);
#   * lk_subscription_booking_router_20260804.func — the same tariff rule for create/join.
#
# The candidate changes exactly one `func` field of each of the three nodes; every other
# node and the `subscriptions_lk1_plan_rules` activation stay byte-identical.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_EVENT_TARIFF_AMOUNTS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_EVENT_TARIFF_AMOUNTS_DEPLOY=CONFIRM_147 npm run nodered:lk1-event-tariff-amounts:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-event-tariff-amounts"
allow_nodes=(lk_subscription_price_preview_20260908_router lk_subscription_price_preview_20260908_evaluate lk_subscription_booking_router_20260804)
...
expected_node_fields='{"lk_subscription_price_preview_20260908_router":["func"],"lk_subscription_price_preview_20260908_evaluate":["func"],"lk_subscription_booking_router_20260804":["func"]}'
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-event-tariff-amounts-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1ta.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_EVENT_TARIFF_AMOUNTS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_tariff_amounts_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.preview?.eventHelpersPublished !== true
    ...
    || value.preview?.evaluateId !== "lk_subscription_price_preview_20260908_evaluate"
    || value.preview?.evaluatorMatchesWritePath !== true
