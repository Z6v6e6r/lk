...
if [[ $# -ne 0 || "${NODE_RED_LK1_PLAN_RULES_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PLAN_RULES_DEPLOY=CONFIRM_147 npm run nodered:lk1-plan-rules:deploy-147" >&2
  exit 2
fi
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-plan-rules-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pr.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PLAN_RULES_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_plan_rules.mjs \
  --workspace "$workspace" \
