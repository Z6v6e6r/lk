...
if [[ -z "${PADLHUB_CUTOVER_GUARDIAN_RECEIPT:-}" || -z "${PADLHUB_CUTOVER_GUARDIAN_RELEASE_REQUEST:-}" || -z "${PADLHUB_CUTOVER_GUARDIAN_RECOVERY_REQUEST:-}" || -z "${PADLHUB_CUTOVER_GUARDIAN_READY_REQUEST:-}" || -z "${PADLHUB_CUTOVER_GUARDIAN_HEARTBEAT:-}" ]]; then
  echo "PADLHUB_CUTOVER_GUARDIAN_RECEIPT, PADLHUB_CUTOVER_GUARDIAN_RELEASE_REQUEST, PADLHUB_CUTOVER_GUARDIAN_RECOVERY_REQUEST, PADLHUB_CUTOVER_GUARDIAN_READY_REQUEST and PADLHUB_CUTOVER_GUARDIAN_HEARTBEAT are required" >&2
  exit 1
fi
...
nohup node "${script_dir}/run_viva_game_projection_fence_guardian.mjs" \
  ...
  --recovery-request "${PADLHUB_CUTOVER_GUARDIAN_RECOVERY_REQUEST}" \
  --ready-request "${PADLHUB_CUTOVER_GUARDIAN_READY_REQUEST}" \
