...
# Guarded deployment of the focused LK1 money-validity diagnostics generation.
#
#   * lk_subscription_booking_router_20260804.func — `lk1Stop` accepts the additive
#     `observed` detail and the annual-HUB money-validity conjunction names the condition
#     it refused on (`stage: "money_validity"`, `violations`, `targetWindowKnown`).
...
if [[ $# -ne 0 || "${NODE_RED_LK1_MONEY_VALIDITY_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_MONEY_VALIDITY_DEPLOY=CONFIRM_147 npm run nodered:lk1-money-validity:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-money-validity"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-money-validity-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1mv.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_MONEY_VALIDITY_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_money_validity_hotfix.mjs \
  --workspace "$workspace" \
