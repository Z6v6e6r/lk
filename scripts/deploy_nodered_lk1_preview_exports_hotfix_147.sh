...
# Guarded deployment of the focused LK1 preview event-helpers hotfix.
#
# Restores the three event-route helpers the 2026-09-15 plan-rules release stopped
# publishing from the preview closure (`identityMoneyOwned`, `lk1LifecycleInstant`,
# `managedExternalEventTypeId`). Without them the router's first guard answered every
# group-training and tournament quote with 503 `*_DISCOUNT_BACKEND_NOT_READY`.
#
# The candidate changes exactly one field of one node:
#   * lk_subscription_price_preview_20260908_router.func — the same reviewed
#     composition, with the three event-route helpers named as extraction roots. The
#     booking gateway, the evaluator and the `subscriptions_lk1_plan_rules`
#     activation stay byte-identical to the installed generation.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PREVIEW_EXPORTS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PREVIEW_EXPORTS_DEPLOY=CONFIRM_147 npm run nodered:lk1-preview-exports:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-preview-event-helpers"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-preview-exports-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pe.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PREVIEW_EXPORTS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_preview_exports_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  const helpers=["identityMoneyOwned","lk1LifecycleInstant","managedExternalEventTypeId"];
  if (value.previewNode?.otherFieldsUnchanged !== true
    || value.previewNode?.initializeUnchanged !== true
    || value.previewNode?.resolverReachable !== true
    || JSON.stringify(value.eventHelpers) !== JSON.stringify(helpers)
    || JSON.stringify(value.previewNode?.eventHelpersPublished) !== JSON.stringify(helpers)
    || !Number.isInteger(value.installedExportedNameCount)) process.exit(1);
' "$candidate_report" "$expected_node_fields" "$expected_changed_nodes"

# Independent exact-graph contract (the candidate changes one `func` only); never
# reuse the patcher's own report.
