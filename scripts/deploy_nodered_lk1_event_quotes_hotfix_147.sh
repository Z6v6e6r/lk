...
# Guarded deployment of the focused LK1 event-quotes generation.
#
# Makes the event route (group training and tournament) produce an actionable price:
#   * lk_subscription_price_preview_20260908_router.func — out-of-contour subscriptions
#     are quoted at the ordinary event tariff instead of being dropped from the batch,
#     plan products are owned by the product identity (the money identity stays HUB-only),
#     decision blockers the client can act on become quotes instead of a 503, every event
#     request drops the previous `msg.responseUrl`, and a refused tariff records the exact
#     sub-condition it refused on;
#   * lk_subscription_price_preview_20260908_final.func — renders that refusal detail;
#   * lk_subscription_booking_router_20260804.func — the same response-provenance fix for
#     create/join, whose tariff check reads the same stale field.
#
# The candidate changes exactly one `func` field of each of the three nodes; the
# evaluator, the split/join bodies and the `subscriptions_lk1_plan_rules` activation
# stay byte-identical to the installed generation.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_EVENT_QUOTES_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_EVENT_QUOTES_DEPLOY=CONFIRM_147 npm run nodered:lk1-event-quotes:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-event-quotes"
allow_nodes=(lk_subscription_price_preview_20260908_router lk_subscription_price_preview_20260908_final lk_subscription_booking_router_20260804)
...
allow_changes=(
  ...
  "lk_subscription_price_preview_20260908_final:func"
  "lk_subscription_booking_router_20260804:func"
)
expected_node_fields='{"lk_subscription_price_preview_20260908_router":["func"],"lk_subscription_price_preview_20260908_final":["func"],"lk_subscription_booking_router_20260804":["func"]}'
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-event-quotes-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1eq.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_EVENT_QUOTES_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_event_quotes_hotfix.mjs \
  --workspace "$workspace" \
...
# Independent exact-graph contract (the candidate changes three `func` fields only);
...
# exists in the preflight receipt. Require the exact reviewed three-node delta and no
# added nodes.
node -e '
  const value=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
  const expected=JSON.parse(process.argv[2]);
