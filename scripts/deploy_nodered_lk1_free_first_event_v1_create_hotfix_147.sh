...
# Guarded deployment of the focused LK1 free-first-event v1-create generation.
#
#   * lk_subscription_booking_router_20260804.func - a visit-covered managed event booking is
#     written through the v1 admin contract (the v2 route answered a subscription-covered
#     create without the booking id the durable operation has to record, so the free first
#     event stayed PENDING_CONFIRMATION while Viva had already written the booking), and the
#     promo variants of РА and Академия join the free-first-event cohort.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_FREE_FIRST_EVENT_V1_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_FREE_FIRST_EVENT_V1_DEPLOY=CONFIRM_147 npm run nodered:lk1-free-first-event-v1-create:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-free-first-event-v1-create"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-free-first-event-v1-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1ffv1.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_FREE_FIRST_EVENT_V1_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_free_first_event_v1_create_hotfix.mjs \
  --workspace "$workspace" \
