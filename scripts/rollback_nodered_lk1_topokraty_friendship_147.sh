...
# Ordered rollback of the LK1 «Дружба Топократы» generation.
#
...
# place while the restored gateway initialize compares against `expectedPrior = null` and the
# restored evaluator has no club branch: the initialize would refuse to start (prior mismatch)
# and a direction-6233 training would be priced at the ordinary 50 %.
#
# Step 1 (this script, first half): deploy the revert candidate — the same club resolver
#   generation with the plan-rules writer replaced by the guarded
...
if [[ "${NODE_RED_LK1_TOPOKRATY_FRIENDSHIP_ROLLBACK:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_TOPOKRATY_FRIENDSHIP_ROLLBACK=CONFIRM_147 npm run nodered:lk1-topokraty-friendship:rollback-147 -- <stamp>" >&2
  exit 2
fi
...
applied_flow_sha="d39a14893e18e8c48000dc8e844f3d1725f3793068c95f85529d549577d80311"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-topokraty-friendship-rollback.XXXXXX)"
ssh_control_root="$(mktemp -d /private/tmp/lk1tkr.XXXXXX)"
