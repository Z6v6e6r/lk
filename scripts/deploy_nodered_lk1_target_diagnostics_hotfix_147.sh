...
# Guarded deployment of the focused LK1 target-diagnostics generation.
#
# Names the sub-condition behind the two anonymous preview refusals that produced the
# 2026-09-17 booking complaints, without changing any accept/reject decision:
#   * lk_subscription_price_preview_20260908_router.func — every target refusal
#     (`*_TARGET_UNRESOLVED`, `PRICE_PREVIEW_GAME_UNRESOLVED`) now answers with
#     `error.details` naming its stage (`event_target`, `game_record`, `game_metadata`,
#     `game_exercise`) and the observed shape: booleans, counts, durations, enum values
#     and Viva type/direction ids only — never an amount, a name, a phone or a booking
#     identity. The patcher proves the generation is diagnostics-only: with the added
#     details stripped, the composed body is byte-identical to the installed one, and the
#     44 refusal call sites are unchanged.
#
# The candidate changes exactly one `func` field of one node; every other node, the
# evaluator, the booking gateway and the `subscriptions_lk1_plan_rules` activation stay
# byte-identical. No node is added and no route changes.
#
...
# the exact live preimage. Everything else fails closed.

set -euo pipefail
umask 077

if [[ $# -ne 0 || "${NODE_RED_LK1_TARGET_DIAGNOSTICS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_TARGET_DIAGNOSTICS_DEPLOY=CONFIRM_147 npm run nodered:lk1-target-diagnostics:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-target-diagnostics"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-target-diagnostics-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1td.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_TARGET_DIAGNOSTICS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_target_diagnostics_hotfix.mjs \
  --workspace "$workspace" \
