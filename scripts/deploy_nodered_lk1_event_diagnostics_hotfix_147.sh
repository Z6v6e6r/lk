...
# Guarded deployment of the focused LK1 event diagnostics generation.
#
# Names the two event-route refusals that still produce 503s, without changing any
# accept/reject decision:
#   * lk_subscription_price_preview_20260908_router.func — the tariff refusal records
#     which amount fields the Viva DTO carried, how many distinct values they hold and
#     how many of them are zero (never an amount itself), and an unmapped decision
#     blocker records the evaluator codes that the preview does not map.
#
# The candidate changes exactly one `func` field of one node; every other node, the
# evaluator and the `subscriptions_lk1_plan_rules` activation stay byte-identical.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_EVENT_DIAGNOSTICS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_EVENT_DIAGNOSTICS_DEPLOY=CONFIRM_147 npm run nodered:lk1-event-diagnostics:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-event-diagnostics"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-event-diagnostics-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1ed.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_EVENT_DIAGNOSTICS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_event_diagnostics_hotfix.mjs \
  --workspace "$workspace" \
