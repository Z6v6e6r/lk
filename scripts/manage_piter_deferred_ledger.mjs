...
export const PITER_DEFERRED_PRIVATE_PATHS = Object.freeze({
  publication: '/root/.node-red/.padlhub-piter-only/release.json',
  startGrant: '/root/.node-red/.padlhub-piter-only/start-grant.json',
  startIntent: '/root/.node-red/.padlhub-piter-only/start-consumed.json',
  startResult: '/root/.node-red/.padlhub-piter-only/start-result.json',
  quiescence: '/root/.node-red/.padlhub-piter-only/quiescence.json',
});
const PUBLICATION_PATH = PITER_DEFERRED_PRIVATE_PATHS.publication;
const START_GRANT_PATH = PITER_DEFERRED_PRIVATE_PATHS.startGrant;
const START_INTENT_PATH = PITER_DEFERRED_PRIVATE_PATHS.startIntent;
const START_RESULT_PATH = PITER_DEFERRED_PRIVATE_PATHS.startResult;
const QUIESCENCE_PROOF_PATH = PITER_DEFERRED_PRIVATE_PATHS.quiescence;
...
export function assertDeferredPrivateDirectory(directory, uid=0, fsImpl=fs) {
  assertOwnedAncestors(path.join(directory, 'custody-probe'),uid,fsImpl);
  const parent=fsImpl.lstatSync(directory);
  if ((parent.mode&0o777)!==0o700) throw new Error('Piter evidence parent must be private 0700');
}

export function publishDeferredStartEvidence(file, payload, uid=0, fsImpl=fs) {
  assertDeferredPrivateDirectory(path.dirname(file),uid,fsImpl);
...
const readStartGrant = uid => {
  assertNoLegacyDeferredStartEvidence();
  assertOwnedAncestors(START_GRANT_PATH,uid);
  return readPrivateDeferredJson(START_GRANT_PATH,uid);
};

export function assertNoLegacyDeferredStartEvidence(fsImpl=fs) {
  for(const file of ['/root/.node-red/.padlhub-piter-only-start-consumed.json',
    '/root/.node-red/.padlhub-piter-only-start-result.json']){
    for(const candidate of [file,file+'.pending']){
      try { fsImpl.lstatSync(candidate); }
      catch(error) { if(error.code==='ENOENT')continue;throw new Error('legacy start evidence custody unknown'); }
      throw new Error('legacy start evidence requires reviewed recovery');
    }
  }
}
...
const readPublication = (packet, expectedUid) => {
  assertNoLegacyDeferredStartEvidence();
  assertDeferredPrivateDirectory(path.dirname(PUBLICATION_PATH),expectedUid);
