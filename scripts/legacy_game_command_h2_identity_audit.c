...
static mount_coverage validate_mount_coverage_file(options *o,const char *mountinfo_path) {
  ...
  FILE *f=fopen(mountinfo_path,"re");if(!f)fail(EXIT_SCAN,"MOUNTINFO_OPEN_FAILED");char *line=NULL;size_t cap=0;uint64_t seen_ids[1024],autofs_ids[1024],binfmt_parent_ids[1024];unsigned char binfmt_exact_source[1024];size_t seen_count=0,autofs_count=0,binfmt_count=0;
  while(getline(&line,&cap,f)>=0){if(strlen(line)>PATH_MAX*4U){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_LINE_TOO_LONG");}char *separator=strstr(line," - ");if(!separator){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_MALFORMED");}*separator='\0';char *right=separator+3,*right_save=NULL;char *fstype=strtok_r(right," \t\r\n",&right_save),*source=strtok_r(NULL," \t\r\n",&right_save),*super_options=strtok_r(NULL," \t\r\n",&right_save);if(!fstype||!source||!super_options){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_MALFORMED");}
    ...
    if(!strcmp(fstype,"binfmt_misc")&&!strcmp(mount_root,"/")&&!strcmp(mount_path,"/proc/sys/fs/binfmt_misc")){if(binfmt_count>=sizeof(binfmt_parent_ids)/sizeof(binfmt_parent_ids[0])){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_TOO_MANY_MOUNTS");}binfmt_parent_ids[binfmt_count]=parent_id;binfmt_exact_source[binfmt_count++]=(unsigned char)!strcmp(source,"binfmt_misc");}
  ...
  if(ferror(f)){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_READ_FAILED");}free(line);fclose(f);if(autofs_count){if(autofs_count!=1)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_AMBIGUOUS");if(binfmt_count==0)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_MISSING");if(binfmt_count!=1)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_AMBIGUOUS");if(binfmt_parent_ids[0]!=autofs_ids[0])fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_MISSING");if(!binfmt_exact_source[0])fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_INVALID");}for(size_t i=0;i<o->mount_count;i++)if(!o->mounts[i].covered)fail(EXIT_SCAN,"MOUNT_SPEC_NOT_IN_MOUNTINFO");finish_sha(&digest,result.inventory_sha256);return result;
}
