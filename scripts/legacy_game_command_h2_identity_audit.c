...
static mount_coverage validate_mount_coverage_file(options *o,const char *mountinfo_path) {
  ...
  if(ferror(f)){free(line);fclose(f);fail(EXIT_SCAN,"MOUNTINFO_READ_FAILED");}free(line);fclose(f);for(size_t i=0;i<autofs_count;i++){size_t matches=0,exact_sources=0;for(size_t n=0;n<binfmt_count;n++)if(binfmt_parent_ids[n]==autofs_ids[i]){matches++;exact_sources+=binfmt_exact_source[n];}if(matches==0)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_MISSING");if(matches!=1)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_AMBIGUOUS");if(exact_sources!=1)fail(EXIT_SCAN,"AUTOFS_PLACEHOLDER_CHILD_INVALID");}for(size_t i=0;i<o->mount_count;i++)if(!o->mounts[i].covered)fail(EXIT_SCAN,"MOUNT_SPEC_NOT_IN_MOUNTINFO");finish_sha(&digest,result.inventory_sha256);return result;
}
