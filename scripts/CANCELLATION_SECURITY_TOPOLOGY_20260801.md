...
Phase two is rebuilt from the exact deployed phase-one preimage after the
frontend cache-bust and live canary:

- builder: `scripts/patch_live_games_split_leave_phase2_cas.mjs`;
- candidate: `/private/tmp/padlhub-cancellation-phase2-cas-20260801-v11.json`;
- report: `/private/tmp/padlhub-cancellation-phase2-cas-20260801-v11.report.json`;
- source SHA-256: `ffe6020341a50755f8f83c35e3099cac87d3b9a6a3aaea6c8c8e9cc26a195ec4`;
...
Both candidates were built from fresh verified pulls. The phase-two report
records five existing-node wire changes and six added CAS nodes; the final
graph SHA intentionally matches the previously reviewed reference while its
provenance now starts at the deployed phase-one SHA. Older intermediate
candidates and reports must not be used.
...
enable PATCH CAS. Phase two must be generated by the dedicated builder against
the exact phase-one SHA and still requires a separate guarded deployment step.
