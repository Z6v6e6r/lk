...
if [[ $# -ne 0 || "${NODE_RED_SUBSCRIPTION_BINDING_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_SUBSCRIPTION_BINDING_DEPLOY=CONFIRM_147 npm run nodered:subscription-binding:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="subscription-binding"
tab_id="4b91e2a2413688db"
...
stage_root="$(mktemp -d /private/tmp/padlhub-subscription-binding-deploy.XXXXXX)"
workspace="$stage_root/live"
candidate_dir="$workspace/candidate-subscription-binding"
source_flow="$workspace/input/source.flow.json"
candidate_flow="$candidate_dir/full.flow.json"
...
cleanup() {
  ...
  rm -f "$workspace/build/selected-tab.nodes.json" "$workspace/build/validation.json" 2>/dev/null || true
  rmdir "$candidate_dir" "$workspace/input" "$workspace/build" "$workspace" "$stage_root" 2>/dev/null || true
}
trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM HUP

bash scripts/prepare_nodered_live_workspace.sh "$workspace" --source-tab-id "$tab_id"
...
node scripts/nodered_reviewed_flow_deploy/prepare_contract.mjs \
  ...
  --allow-node e92e68bf3f08a70c \
  --allow-node 8f7bd5b482fe9763 \
  --allow-node legacy_payment_confirm_canonical_prepare_20260816 \
  --allow-node legacy_roster_bridge_build_20260816 \
  --allow-node 7c280001a0c1e015 \
  --allow-node 9878400d518ebcbd >/dev/null
...
node -e '
  ...
  if (!value.ok || value.action !== "preflight" || value.sourceSha256 !== process.argv[2] || value.candidateSha256 !== process.argv[3] || value.changedNodeCount !== 6) process.exit(1);
...
node -e '
  const value=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
  if (!value.ok || value.action !== "apply" || value.sourceSha256 !== process.argv[2] || value.activeFlowSha256 !== process.argv[3] || value.flowBackup !== process.argv[4] || value.contractBackup !== process.argv[5]) process.exit(1);
...
if [[ "$public_games_status" != "200" ]]; then
  echo "Public games postcheck failed; automatic rollback requested" >&2
  exit 5
fi
