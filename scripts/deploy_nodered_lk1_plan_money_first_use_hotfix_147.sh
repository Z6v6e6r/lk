...
# Guarded deployment of the focused LK1 plan-money first-use generation.
#
#   * lk_subscription_booking_router_20260804.func - the money mandate on the direct
#     group-training route now produces its readback evidence for exactly the cohort
#     `resolveLk1Rule` marks enforced (the product-identity projection rejected every
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PLAN_MONEY_FIRST_USE_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PLAN_MONEY_FIRST_USE_DEPLOY=CONFIRM_147 npm run nodered:lk1-plan-money-first-use:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-plan-money-first-use"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-plan-money-first-use-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pmfu.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PLAN_MONEY_FIRST_USE_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_plan_money_first_use_hotfix.mjs \
  --workspace "$workspace" \
