...
# Guarded deployment of the focused LK1 PRO-training Energy visit-pack generation.
#
#   * lk_subscription_booking_router_20260804.func - the reviewed PRO-training module
#     (scripts/lib/proTrainingExclusion.mjs) embedded once, plus the reviewed refusal and
#     exact Energy 5/25 exception. Managed discounts and free-first-event remain blocked;
#     an owned Energy pack follows the legacy visit path.
#   * lk_subscription_price_preview_20260908_router.func - recomposed from the patched
#     generation, so the advisory quote and the write path agree, and the reconciliation
#     between the bounded candidate and the installed flow is bounded per-node;
#     the preview carries the real PRO predicate, never the inert stub.
#
# The candidate changes exactly two nodes (two `func` fields); every other node, route and
# the existing activation globals stay byte-identical, and no node is added.
#
# This generation is stacked on the installed PRO exclusion generation; its reviewed full-flow
# preimage is `bcc9fd6b…`, so a stale or independently changed live flow fails closed.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PRO_TRAINING_ENERGY_EXCLUSIONS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PRO_TRAINING_ENERGY_EXCLUSIONS_DEPLOY=CONFIRM_147 npm run nodered:lk1-pro-training-energy-exclusions:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-pro-training-energy-exclusions"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-pro-training-energy-exclusions-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pte.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PRO_TRAINING_ENERGY_EXCLUSIONS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
# Full-flow preimage: the focused generation pins the exact live flow sha, the installed
# booking func and setup bodies and the installed preview body, so a tab-scoped
# extraction is not enough.
pull_live_workspace

mkdir -m 700 "$candidate_dir"
node scripts/patch_live_lk1_pro_training_energy_exclusions_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    ...
    || value.booking?.refusalPrecedesContour !== true
    || value.preview?.id !== "lk_subscription_price_preview_20260908_router"
    || value.preview?.refusalBound !== true
    || value.preview?.inertStubAbsent !== true
    || value.preview?.initializeUnchanged !== true) process.exit(1);
' "$candidate_report" "$expected_node_fields" "$expected_changed_nodes"

# Independent exact-graph contract (the candidate changes three fields of two nodes);
...
# The installed booking body must carry the reviewed PRO refusal, Energy allowlist and module,
# and the installed preview must carry the real predicate (not the inert stub). The byte
# readback above proves what was written; this proves the marker the contour decision
# depends on is present in the node the runtime actually executes.
if ! remote_ssh "node -e 'const fs=require(\"node:fs\");const flow=JSON.parse(fs.readFileSync(\"/root/.node-red/flows.json\",\"utf8\"));const booking=flow.find(row=>row.id===\"lk_subscription_booking_router_20260804\");const preview=flow.find(row=>row.id===\"lk_subscription_price_preview_20260908_router\");const func=booking&&typeof booking.func===\"string\"?booking.func:\"\";const previewFunc=preview&&typeof preview.func===\"string\"?preview.func:\"\";const wanted=[\"PRO_TRAINING_SUBSCRIPTION_UNAVAILABLE\",\"function isProTrainingExercise(value) {\",\"function isProTrainingEnergyPack(value) {\",\"PRO_TRAINING_ENERGY_PRODUCT_IDS\",\"isProTrainingEnergyPack(selectedOwned[0])\"];if(!wanted.every(marker=>func.includes(marker)))process.exit(1);if(!previewFunc.includes(\"canonical.isProTrainingExercise\")||previewFunc.includes(\"const isProTrainingExercise = () => false;\"))process.exit(1);'"; then
  echo "Installed nodes do not carry the reviewed PRO-training refusal; automatic rollback requested" >&2
  exit 7
fi
...
echo "proTrainingDirections=5502,5503,5504,5505,5506,5507"
echo "changedNodeCount=$expected_changed_nodes"
echo "refusalCode=PRO_TRAINING_SUBSCRIPTION_UNAVAILABLE"
echo "flowBackup=$remote_flow_backup"
echo "contractBackup=$remote_contract_backup"
