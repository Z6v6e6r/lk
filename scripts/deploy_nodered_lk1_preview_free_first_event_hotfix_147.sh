...
# Guarded deployment of the focused LK1 preview free-first-event generation.
#
#   * lk_subscription_booking_router_20260804.func - the client discount expectation is compared
#     against the decision, so a visit-covered first event of the day (FREE_ENTITLEMENT at zero)
#     accepts both the full-benefit quote and a bundle's older configured-discount quote instead
#     of answering 409 GROUP_DISCOUNT_QUOTE_CHANGED;
#   * lk_subscription_price_preview_20260908_router.func - the preview claims the same day
#     snapshot the booking gateway proves (cohort table, operation category, proven visit
#     balance) and quotes the free covered event as the full benefit at zero.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PREVIEW_FREE_FIRST_EVENT_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PREVIEW_FREE_FIRST_EVENT_DEPLOY=CONFIRM_147 npm run nodered:lk1-preview-free-first-event:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-preview-free-first-event"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-preview-free-first-event-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pffe.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PREVIEW_FREE_FIRST_EVENT_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_preview_free_first_event_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    ...
    || value.booking?.freeFirstExpectationNormalized !== true
    || value.booking?.reviewedExpectationUntouched !== true) process.exit(1);
  if (value.preview?.id !== "lk_subscription_price_preview_20260908_router"
    || value.preview?.otherFieldsUnchanged !== true
    || value.preview?.daySnapshotClaimed !== true
    || value.preview?.freeQuoteEmitted !== true) process.exit(1);
