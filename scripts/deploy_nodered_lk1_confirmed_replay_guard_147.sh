...
# Guarded deployment of the focused LK1 confirmed-replay guard generation.
#
# Publishes one reviewed flow that changes exactly two `func` fields: the subscription
# gateway verifies the provider booking before it replays a CONFIRMED claim (and releases
# the claim when the booking is gone), and the subscription finalizer adds the money
# evidence and the intent identity the widget validates on a replay.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_CONFIRMED_REPLAY_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_CONFIRMED_REPLAY_DEPLOY=CONFIRM_147 npm run nodered:lk1-confirmed-replay:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-confirmed-replay-guard"
allow_nodes=(lk_subscription_booking_router_20260804 lk_subscription_booking_finalize_20260804)
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-readback-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1rb.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_CONFIRMED_REPLAY_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
# Full-flow preimage: the focused generation validates the exact live function
# body, so a tab-scoped extraction is not enough.
pull_live_workspace
...
node -e '
  ...
  const expected=process.argv.slice(3).slice().sort();
  if (!changes || changes.length !== Number(process.argv[2])
    || changes.some((change) => JSON.stringify(change.fields) !== JSON.stringify(["func"]))
    || JSON.stringify(changes.map((change) => change.id).sort()) !== JSON.stringify(expected)) process.exit(1);
...
# the installed-flow readback above): the paid-join readback itself needs a real
# participant join. apply restarts Node-RED, so the endpoint may answer with an
# error page for a short window; retry within a bounded budget and fail closed if
# the payload never matches.
smoke_ok=0
for ((smoke_attempt = 1; smoke_attempt <= 30; smoke_attempt++)); do
  if curl -sS --max-time 15 "$smoke_url" -o "$postcheck_result" \
    && node -e '
        const value=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
