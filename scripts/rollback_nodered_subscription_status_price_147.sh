...
# Guarded rollback of the focused subscription status-price generation.
...
if [[ $# -ne 1 || "${NODE_RED_SUBSCRIPTION_STATUS_PRICE_ROLLBACK:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_SUBSCRIPTION_STATUS_PRICE_ROLLBACK=CONFIRM_147 npm run nodered:subscription-status-price:rollback-147 -- YYYYMMDDTHHMMSS+ZZZZ" >&2
  exit 2
fi
...
public_status="$(curl --retry 5 --retry-delay 2 --retry-connrefused --max-time 20 -sS -o /dev/null -w '%{http_code}' 'https://padlhub.su/lk/tournaments/summer-subscription/status?counterKey=ra' || true)"
