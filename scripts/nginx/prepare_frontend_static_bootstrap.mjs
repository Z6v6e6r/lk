...
export function prepareBootstrap({ sourceNginx, expectedSourceSha, installed, distDir, fontsDir, outDir }) {
  ...
  const plan = { schema: 'LK_FRONTEND_STATIC_BOOTSTRAP_V1', installed,
    ...
    activePath: CURRENT_ROOT, publicLinkTarget: 'lk-frontend-releases/current',
    mutableCurrentPath: '/var/www/html/lk-frontend-releases/current', retainedReleasePath: `/var/www/html/lk-frontend-releases/${installed.source}-${releaseDigest.slice(0, 16)}`,
    ...
    gates: ['Verify current nginx and all installed artifact preimages under one approved writer boundary',
      'Install retained baseline, store/current symlink and root-owned immutable public symlink without modifying legacy lk',
