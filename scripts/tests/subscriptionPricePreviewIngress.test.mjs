...
import {buildSubscriptionPricePreviewNginxCandidate,previewLocation} from '../nginx/prepare_subscription_price_preview.mjs';
...
test('real isolated nginx rejects burst/body overflow, preserves CORS, and lets OPTIONS bypass rate limit',
 {skip:process.env.LK_PRICE_PREVIEW_NGINX_TEST!=='1'&&'Opt-in isolated Docker runtime fixture',timeout:60000},async t=>{
  ...
  fs.writeFileSync(join(root,'nginx.conf'),`pid /tmp/nginx.pid; error_log stderr warn; events {} http { access_log off; ${guard}
