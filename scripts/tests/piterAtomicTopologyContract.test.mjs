...
function validFlow() {
  ...
  return [
    ...
    { id: ids.atomicRouter, type: "function", z: ids.tab, name: "Route atomic Piter subscription sale", func: atomicRouterSource, outputs: 5, timeout: "", noerr: 0, initialize: PITER_ATOMIC_BINDING_INITIALIZER_SOURCE, finalize: "", libs: [], x: 2750, y: 2240, wires: [[ids.ledgerFind], [ids.ledgerUpdate], [ids.saleUpdate], [ids.response], [ids.viva]] },
...
test("Piter atomic initializer sets only the exact missing or matching product binding", () => {
  const key = "summer_subscription_piter_friendship_product_id";
  const productId = "8bf334ba-3050-4017-b40a-7eef2db1eb16";
  for (const initial of [undefined, null, "", productId]) {
    const values = new Map(initial === undefined ? [] : [[key, initial]]);
    vm.runInNewContext(PITER_ATOMIC_BINDING_INITIALIZER_SOURCE, {
      global: {
        get: (name) => values.get(name),
        set: (name, value) => values.set(name, value),
      },
    });
    assert.equal(values.get(key), productId);
  }

  for (const conflictingValue of ["different-product", false, 0]) {
    const values = new Map([[key, conflictingValue]]);
    assert.throws(() => vm.runInNewContext(PITER_ATOMIC_BINDING_INITIALIZER_SOURCE, {
      global: {
        get: (name) => values.get(name),
        set: (name, value) => values.set(name, value),
      },
    }), /product binding mismatch/);
    assert.equal(values.get(key), conflictingValue);
  }
});
...
test("exact Piter atomic topology rejects graph, Mongo, function, and debug drift", () => {
  ...
  for (const mutate of [
    ...
    (flow) => { flow.find(({ id }) => id === ids.atomicRouter).initialize = "global.set('wrong', true);"; },
