...
# Guarded deployment of the focused LK1 paid-join readback generation.
#
# Publishes one reviewed flow whose only change is the `lk1_transaction_readback`
# step of the subscription gateway function body: the checkout link and the
# client/booking evidence are read from the real Viva transaction DTO.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PAYMENT_READBACK_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PAYMENT_READBACK_DEPLOY=CONFIRM_147 npm run nodered:lk1-payment-readback:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-payment-readback"
...
ssh_retry_attempts="${NODE_RED_LK1_PAYMENT_READBACK_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_payment_readback.mjs \
  --workspace "$workspace" \
