        ...
        if (url.pathname === '/') {
          ...
          res.end(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>LK1 — локальная версия</title><style>
            *{box-sizing:border-box}html,body{margin:0;height:100%;font-family:system-ui,sans-serif;background:#f4f6f8;color:#152b3a}
            body{display:flex;flex-direction:column}header{padding:12px 20px;background:#152b3a;color:white;display:flex;gap:8px 20px;align-items:center;flex-wrap:wrap}strong{white-space:nowrap}button{min-height:44px;padding:8px 16px;border:0;border-radius:8px;cursor:pointer;font:inherit;background:white;color:#152b3a}button:focus-visible{outline:3px solid #ffda00;outline-offset:3px}button:disabled{opacity:.6;cursor:wait}span{font-size:13px;line-height:1.4;color:#dde6ee}iframe{width:100%;flex:1;border:0;background:white}
          </style></head><body><header><strong>LK1 · Локальная версия</strong><span>Вход через Яндекс или SMS · Боевые данные только для чтения. Брони, оплаты и изменения заблокированы.</span><button id="lk1-yandex" type="button">Войти через Яндекс</button><span id="lk1-oauth-status" role="status"></span></header><script src="/__lk1_local/oauth.js"></script><iframe title="Личный кабинет LK1" src="/lk_new?authMode=viva" sandbox="allow-scripts allow-same-origin allow-forms"></iframe></body></html>`);
          return;
        }
