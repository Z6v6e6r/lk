...
# Guarded deployment of the focused LK1 station-exclusions generation.
#
#   * lk_subscription_booking_router_20260804.func - the reviewed resolver module and
#     `lk1Config` fragment, and the booking target's station at every contour decision
#     (quote, money gate, checkout re-resolution, both ingress gates, product-identity
#     projection), so the pair «Sirius studio x friendship product» resolves to the
#     pre-rollout legacy verdict while every other product and station keep their rule;
#   * lk_subscription_booking_router_20260804.initialize - the reviewed
#     `subscriptions_lk1_station_exclusions` writer appended after the untouched
#     HUB-policy and plan-rules writers;
#   * lk_subscription_price_preview_20260908_router.func - recomposed from the patched
#     generation, so the preview and the write path price the same verdict.
#
# The candidate changes exactly two nodes (three fields); every other node, route and
# the existing activation globals stay byte-identical.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_STATION_EXCLUSIONS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_STATION_EXCLUSIONS_DEPLOY=CONFIRM_147 npm run nodered:lk1-station-exclusions:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-station-exclusions"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-station-exclusions-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1ste.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_STATION_EXCLUSIONS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_station_exclusions_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    || value.booking?.stationCallSites !== true
    || value.booking?.stationExclusionsBound !== true
    || value.booking?.stationWriterBound !== true
    || value.booking?.planRulesWriterKept !== true
    || value.booking?.hubPolicyWriterKept !== true
    || value.preview?.id !== "lk_subscription_price_preview_20260908_router"
    || value.preview?.stationAware !== true
...
# The installed setup must carry the reviewed exclusion payload: the byte readback
# above proves what was written, this proves the activation the contour decision
# depends on is present with the exact reviewed pair. A flow that installs without it
# would keep charging the Sirius pair the 50 % co-pay while looking green (review F6).
if ! remote_ssh "node -e 'const fs=require(\"node:fs\");const flow=JSON.parse(fs.readFileSync(\"/root/.node-red/flows.json\",\"utf8\"));const node=flow.find(row=>row.id===\"lk_subscription_booking_router_20260804\");const init=node&&typeof node.initialize===\"string\"?node.initialize:\"\";const wanted=[\"subscriptions_lk1_station_exclusions\",\"233c1405-1eac-40de-8ec6-1cf7e24c9276\",\"b2e6a9d4-53b5-4f79-87ec-3fb076381e9b\",\"global.set(lk1StationExclusionsKey, lk1DesiredStationExclusions);\",\"station exclusions readback mismatch\"];if(!wanted.every(marker=>init.includes(marker)))process.exit(1);'"; then
  echo "Installed setup does not carry the reviewed station-exclusions activation; automatic rollback requested" >&2
  exit 7
fi
...
echo "exclusionPair=233c1405-1eac-40de-8ec6-1cf7e24c9276:b2e6a9d4-53b5-4f79-87ec-3fb076381e9b"
