...
# Guarded deployment of the focused LK1 money-evidence diagnostics generation.
#
#   * lk_subscription_booking_router_20260804.func — the quote-level
#     `LK1_MONEY_SUBSCRIPTION_VALIDITY_UNPROVEN` stop now reports why the mandate was not
#     proven in this pass (`stage: "money_evidence"`, `evidencePresent`, `readbackPhase`,
#     `selectedOwned`, `eventCategory`). The verdict itself is unchanged and every other
#     code keeps the previous `{ code }` body.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_MONEY_EVIDENCE_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_MONEY_EVIDENCE_DEPLOY=CONFIRM_147 npm run nodered:lk1-money-evidence:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-money-evidence"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-money-evidence-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1me.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_MONEY_EVIDENCE_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_money_evidence_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    ...
    || value.booking?.diagnosticsPresent !== true) process.exit(1);
