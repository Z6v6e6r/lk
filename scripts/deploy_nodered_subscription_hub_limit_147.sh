...
# Guarded deployment of the focused annual-HUB daily-limit generation.
# Publishes one reviewed flow whose only change is the four counter function
# bodies that read the annual daily seat count from configuration.
#
...
if [[ $# -ne 0 || "${NODE_RED_SUBSCRIPTION_HUB_LIMIT_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_SUBSCRIPTION_HUB_LIMIT_DEPLOY=CONFIRM_147 npm run nodered:subscription-hub-daily-limit:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="subscription-hub-daily-limit"
allow_nodes=(8fdc7076a0c436a2 c165e43eba668c25 91dded2dc8cfebe4 519b6a6ca208e281)
expected_price_minor="9800000"
...
candidate_dir="$workspace/candidate-hub-limit"
...
ssh_retry_attempts="${NODE_RED_SUBSCRIPTION_HUB_LIMIT_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node -e '
  ...
  if (!value.ok || value.action !== "preflight" || value.sourceSha256 !== process.argv[2]
    || value.candidateSha256 !== process.argv[3] || value.changedNodeCount !== 4) process.exit(1);
...
# Functional smoke for the storefront. This is NOT the deploy proof (see the
# installed-flow readback above): the daily seat may legitimately be taken, so
# only reachability, the counter identity and the published price are asserted.
...
for ((smoke_attempt = 1; smoke_attempt <= 30; smoke_attempt++)); do
  if curl -sS --max-time 15 \
    'https://padlhub.su/lk/tournaments/summer-subscription/status?counterKey=network_friendship' -o "$postcheck_result" \
    && node -e '
        const value=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
        const status=Array.isArray(value) ? value[0] : value;
        if (!status || status.counterKey !== "network_friendship"
...
if [[ "$smoke_ok" != "1" ]]; then
  echo "HUB status smoke check failed after the warm-up budget; automatic rollback requested" >&2
  exit 6
fi
...
echo "flowBackup=$remote_flow_backup"
echo "contractBackup=$remote_contract_backup"
echo "hubPriceMinor=$expected_price_minor"
