...
function validateSubscriptionSalePeriodHistory(publications) {
    ...
    const ordered = candidates.sort((left, right) => left.effectiveAt - right.effectiveAt);
 ...
 */
function resolveSubscriptionSalePeriod(input) {
    ...
    const startsAtOrBeforePurchase = history.publications.filter((publication) => Date.parse(publication.effectiveAt) <= purchasedAt);
    if (startsAtOrBeforePurchase.length === 0)
        return { matchCount: 0, kind: 'NO_MATCH' };
    const latestStart = Math.max(...startsAtOrBeforePurchase.map((publication) => Date.parse(publication.effectiveAt)));
    const matches = startsAtOrBeforePurchase.filter((publication) => Date.parse(publication.effectiveAt) === latestStart);
    if (matches.length !== 1)
        return { matchCount: matches.length, kind: 'AMBIGUOUS' };
    return { matchCount: 1, kind: 'MATCH', publication: matches[0] };
}
output = resolveSubscriptionSalePeriod(input);
