...
# Guarded deployment of the focused LK1 PRO-training exclusion generation.
#
...
#     selected instance, the money readback, the plan rule and any write. A PRO training
#     (directions 5502-5507) can no longer be booked or discounted through any subscription;
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PRO_TRAINING_EXCLUSIONS_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PRO_TRAINING_EXCLUSIONS_DEPLOY=CONFIRM_147 npm run nodered:lk1-pro-training-exclusions:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-pro-training-exclusions"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-pro-training-exclusions-deploy.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PRO_TRAINING_EXCLUSIONS_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_pro_training_exclusions_hotfix.mjs \
  --workspace "$workspace" \
...
# The installed booking body must carry the reviewed PRO refusal and the reviewed module,
