...
if [[ $# -ne 0 || "${NODE_RED_LK1_PRO_ENERGY_SUFFIX_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PRO_ENERGY_SUFFIX_DEPLOY=CONFIRM_147 bash scripts/deploy_nodered_lk1_pro_energy_suffix_147.sh" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-pro-energy-suffix"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-pro-energy-suffix-deploy.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PRO_ENERGY_SUFFIX_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_pro_energy_suffix.mjs \
  --workspace "$workspace" \
