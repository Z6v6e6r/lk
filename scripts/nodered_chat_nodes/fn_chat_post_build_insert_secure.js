...
const game = rows[0] || {};
const ctx = msg._chat || {};
const actor = { clientId: normalizeId(ctx.senderId), phone: normPhone(ctx.senderPhone) };
...
const activePhones = uniq(members.map((member) => normPhone(
  member?.phoneNorm || member?.phone || member?.mobile || member?.clientPhoneNorm || member?.clientPhone,
)));
const now = new Date();
const metadata = isObj(game?.metadata) ? game.metadata : {};
const senderRole = [
  game?.organizer,
  { id: metadata.organizerId, phoneNorm: metadata.organizerPhoneNorm || metadata.organizerPhone },
].some((member) => memberMatches(member, actor)) ? "ORGANIZER" : "PLAYER";
const messageDoc = {
  gameId: game.id,
  tenantKey: game.tenantKey || null,
  relatedPhones: activePhones,
  sender: {
