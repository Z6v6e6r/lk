...
# Guarded deployment of the focused LK1 preview multi-product generation.
#
#   * lk_subscription_price_preview_20260908_router.func - the preview validates its operation
#     batch against the scope its Mongo query used (`ctx.ruleProductIds`) instead of one rule
#     product, so a client who owns an annual HUB and a plan product at the same time gets a
#     price quote instead of a 503 LK1_ALLOWANCE_RECORD_INVALID.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_PREVIEW_MULTI_PRODUCT_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_PREVIEW_MULTI_PRODUCT_DEPLOY=CONFIRM_147 npm run nodered:lk1-preview-multi-product:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-preview-multi-product"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-preview-multi-product-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1pmp.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_PREVIEW_MULTI_PRODUCT_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_preview_multi_product_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_price_preview_20260908_router"
    || value.booking?.otherFieldsUnchanged !== true
    || value.booking?.batchScopeMatchesQuery !== true
    || value.booking?.usageBuilderUnchanged !== true) process.exit(1);
