...
# Guarded deployment of the focused LK1 free-first-event generation.
#
#   * lk_subscription_booking_router_20260804.func - the covered cohort (РА and Академия)
#     proves the first event of the subscription's local service day and the visit balance
#     in the managed policy input, and the discounted checkout re-resolves the stored rule
#     with its sale date (a plan rule cannot select its cohort without it, so the checkout
#     was refused after the booking had already been written);
#   * lk_subscription_managed_policy_20260820.func and
#     lk_subscription_price_preview_20260908_evaluate.func - the same evaluator body turns a
#     proved first event into FREE_ENTITLEMENT with one visit and keeps the configured
#     discount for every later event of the day or an exhausted visit balance.
#
# The candidate changes exactly one `func` field of three nodes; every other node and the
...
if [[ $# -ne 0 || "${NODE_RED_LK1_FREE_FIRST_EVENT_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_FREE_FIRST_EVENT_DEPLOY=CONFIRM_147 npm run nodered:lk1-free-first-event:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-free-first-event"
allow_nodes=(lk_subscription_booking_router_20260804 lk_subscription_managed_policy_20260820 lk_subscription_price_preview_20260908_evaluate)
...
expected_node_fields='{"lk_subscription_booking_router_20260804":["func"],"lk_subscription_managed_policy_20260820":["func"],"lk_subscription_price_preview_20260908_evaluate":["func"]}'
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-free-first-event-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1ffe.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_FREE_FIRST_EVENT_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_free_first_event_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    ...
    || value.booking?.freeFirstCohortBound !== true
    || value.booking?.dayBucketProved !== true
    || value.booking?.checkoutCohortBound !== true
    || value.evaluator?.otherFieldsUnchanged !== true
    || value.evaluator?.freeFirstBenefitBound !== true
    || value.evaluator?.discountUnchanged !== true) process.exit(1);
