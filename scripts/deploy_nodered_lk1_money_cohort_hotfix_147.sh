...
# Guarded deployment of the focused LK1 money-cohort generation.
#
#   * lk_subscription_booking_router_20260804.func - the annual-HUB money mandate now
#     produces its readback evidence for exactly the cohort `resolveLk1Rule` marks
#     enforced. The leftover pre-plan-rules sale-date gate skipped the proof for an annual
#     HUB sold before 2026-09-01 while `lk1Quote` demanded it, so every create/join on the
#     event route for that cohort was refused with
#     LK1_MONEY_SUBSCRIPTION_VALIDITY_UNPROVEN.
#
...
if [[ $# -ne 0 || "${NODE_RED_LK1_MONEY_COHORT_DEPLOY:-}" != "CONFIRM_147" ]]; then
  echo "Usage: NODE_RED_LK1_MONEY_COHORT_DEPLOY=CONFIRM_147 npm run nodered:lk1-money-cohort:deploy-147" >&2
  exit 2
fi

host="lk-primary-147"
deployment_id="lk1-money-cohort"
...
stage_root="$(mktemp -d /private/tmp/padlhub-lk1-money-cohort-deploy.XXXXXX)"
# Unix socket paths are limited to about 104 bytes, so keep the control path short.
ssh_control_root="$(mktemp -d /private/tmp/lk1mc.XXXXXX)"
...
ssh_retry_attempts="${NODE_RED_LK1_MONEY_COHORT_SSH_ATTEMPTS:-5}"
retry_idempotent() {
  local attempt
...
node scripts/patch_live_lk1_money_cohort_hotfix.mjs \
  --workspace "$workspace" \
...
node -e '
  ...
  if (value.booking?.id !== "lk_subscription_booking_router_20260804"
    ...
    || value.booking?.cohortGateRemoved !== true) process.exit(1);
